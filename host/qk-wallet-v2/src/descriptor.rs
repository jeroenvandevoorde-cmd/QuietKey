//! Private construction of the ratified v2 two-account descriptor pair.
...
pub(crate) struct DerivedWalletV2 {
...
impl<const N: usize> Writer<N> {
    ...
    fn push(&mut self, input: &[u8]) -> Result<(), WalletV2Error> {
        let end = self
            .len
...
fn checksum(body: &[u8; BODY_BYTES]) -> Result<[u8; 8], WalletV2Error> {
    let mut state = 1u64;
    let mut class = 0u8;
    ...
    for &byte in body {
        let position = INPUT_CHARSET
            ...
            .ok_or(WalletV2Error::CryptographicInvariant)? as u8;
...
fn build_one(
    accounts: &[AccountPublic; 2],
    branch: u8,
) -> Result<[u8; DESCRIPTOR_BYTES], WalletV2Error> {
...
pub(crate) fn build_wallet_v2(
    accounts: [AccountPublic; 2],
) -> Result<DerivedWalletV2, WalletV2Error> {
    let receive = build_one(&accounts, 0)?;
    let change = build_one(&accounts, 1)?;
    let pair = parse_descriptor_pair_v2(&receive, &change)
        .map_err(|_| WalletV2Error::GeneratedDescriptorInvalid)?;
    let receive_derived = derive_receive_script_v2(&pair, 0)
        .map_err(|_| WalletV2Error::GeneratedDescriptorInvalid)?;
    let change_derived =
        derive_change_script_v2(&pair, 0).map_err(|_| WalletV2Error::GeneratedDescriptorInvalid)?;
    let receive_program: [u8; 32] = receive_derived.script_pubkey[2..]
        .try_into()
        .map_err(|_| WalletV2Error::CryptographicInvariant)?;
...
mod tests {
    ...
    fn duplicate_generated_v2_accounts_are_a_named_descriptor_rejection() {
        ...
        assert!(matches!(
            build_wallet_v2(accounts),
            Err(WalletV2Error::GeneratedDescriptorInvalid)
        ));
    }
}

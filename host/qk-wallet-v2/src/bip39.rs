...
const PBKDF2_SALT_CAPACITY: usize = 8 + MAX_PASSPHRASE_BYTES + 4;
...
fn entropy_to_mnemonic(entropy: &[u8; ENTROPY_BYTES]) -> Result<Mnemonic, WalletV2Error> {
    ...
    for word_number in 0..MNEMONIC_WORDS {
        ...
        let word = word_at(index).ok_or(WalletV2Error::CryptographicInvariant)?;
        if word_number != 0 {
...
) -> Result<Secret<64>, WalletV2Error> {
    if !password.is_ascii() || !passphrase_nfkd_ascii.is_ascii() {
        return Err(WalletV2Error::CryptographicInvariant);
    }
    if passphrase_nfkd_ascii.len() > MAX_PASSPHRASE_BYTES {
        return Err(WalletV2Error::CryptographicInvariant);
    }
    let mut salt = [0u8; PBKDF2_SALT_CAPACITY];
    salt[..8].copy_from_slice(b"mnemonic");
    let passphrase_end = 8 + passphrase_nfkd_ascii.len();
    salt[8..passphrase_end].copy_from_slice(passphrase_nfkd_ascii);
    let salt_len = passphrase_end + 4;
    salt[passphrase_end..salt_len].copy_from_slice(&1u32.to_be_bytes());
    let mut u = [0u8; 64];
    hmac_sha512_into(password, &salt[..salt_len], &mut u);
...
pub(crate) fn entropy_to_seed(entropy: &[u8; 32]) -> Result<Secret<64>, WalletV2Error> {
...
pub(crate) fn vector_entropy_to_mnemonic_and_seed(
    entropy: &[u8; 32],
    passphrase: &[u8],
) -> Result<(Vec<u8>, [u8; 64]), WalletV2Error> {
    let mnemonic = entropy_to_mnemonic(entropy)?;
...
mod tests {
    ...
    const FIXTURE: &str =
        include_str!("../../qk-provisioning/tests/fixtures/bip39-english-256.txt");

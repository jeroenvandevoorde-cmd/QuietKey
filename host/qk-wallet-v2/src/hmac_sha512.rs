//! Private FIPS 198-1 HMAC-SHA512 reused on the QK-DEC-048 pattern.
//!
...
const B: usize = 128;
/// Largest private message used by the inherited vector span.
...
pub(crate) fn hmac_sha512_into(key: &[u8], message: &[u8], result: &mut [u8; 64]) {
    ...
    let mut inner = [0u8; B + MAX_MESSAGE_BYTES];
    for (slot, &byte) in inner[..B].iter_mut().zip(k0.iter()) {
        *slot = byte ^ 0x36;
    }
    let inner_len = B + message.len();
    inner[B..inner_len].copy_from_slice(message);
    let mut inner_hash = [0u8; 64];
    sha512_into(&inner[..inner_len], &mut inner_hash);
...
mod tests {
    ...
    // never instructions.
    const HMAC_RSP: &[u8] = include_bytes!("../../qk-bip32/tests/fixtures/HMAC-SHA512.rsp");

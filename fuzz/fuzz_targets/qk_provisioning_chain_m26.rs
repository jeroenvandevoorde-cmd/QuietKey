...
enum Outcome {
    ...
    Complete(ProvisioningArtifacts),
}
...
fn transcripts(data: &[u8]) -> [[u8; 100]; 4] {
    ...
    for (role, transcript) in output.iter_mut().enumerate() {
        for (position, symbol) in transcript.iter_mut().enumerate() {
            let fallback = (role * 97 + position * 29 + 11) as u8;
            let value = if data.is_empty() {
                fallback
            } else {
                data[(role * 100 + position) % data.len()].wrapping_add(fallback)
            };
            *symbol = b'1' + value % 6;
        }
        for symbol in transcript.iter_mut().take(4) {
            *symbol = b'1' + role as u8;
        }
    }
    output
}
...
fn nonce(data: &[u8]) -> [u8; 12] {
    ...
    for (index, slot) in nonce.iter_mut().enumerate() {
        *slot = data
            .get(index)
            .copied()
            .unwrap_or((index as u8).wrapping_mul(17).wrapping_add(0x26));
    }
    nonce
}
...
fn assert_address(address: &[u8; 62], script_pubkey: &[u8; 34]) {
    ...
    assert_eq!(polymod, 1);
    assert_eq!(symbols[0], 0, "only witness version zero is in scope");
    ...
    assert_eq!(bits, 4);
    assert_eq!(accumulator & 0x0f, 0, "Bech32 padding must be zero");
...
fn assert_artifacts(
    ...
    assert_ne!(artifacts.account_xpubs[0], artifacts.account_xpubs[2]);
    assert_ne!(artifacts.account_xpubs[1], artifacts.account_xpubs[2]);

    let pair = parse_descriptor_pair(&artifacts.descriptors[0], &artifacts.descriptors[1])
        .expect("M26 output descriptors must strictly reparse");
    assert_eq!(pair.wallet_id(), artifacts.wallet_id);
    let receive = derive_receive_script(&pair, 0).expect("first receive derivation");
    let change = derive_change_script(&pair, 0).expect("first change derivation");
    ...
    .expect("M26 capsule must authenticate under its public dice-derived A2");
...
fn run_once(data: &[u8]) -> Outcome {
    ...
    assert_eq!(
        ...
        "a completed run must never emit a second partial artifact"
    );

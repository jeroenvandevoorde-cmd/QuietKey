...
target=$1
case "$target" in
  qk_sec1210_wire) max_len=4096; seed=167010 ;;
  qk_t1) max_len=8192; seed=167009 ;;
...
case "$target" in
  ...
  qk_supervisor_process_lifecycle)
    set -- --no-default-features --features process-s8-supervisor "$target" "fuzz/corpus/$target"
    ;;
  qk_decoy_calculator)
  ...
  qk_core_kit_intake|qk_core_kit_restore|qk_core_kit_spend)
    set -- --no-default-features --features process-s7-core "$target" "fuzz/corpus/$target"
    ;;
  qk_card_protocol)
    set -- --no-default-features --features card-s1-protocol "$target" "fuzz/corpus/$target"
    ;;
  qk_card_model)
    set -- --no-default-features --features card-s1-model "$target" "fuzz/corpus/$target"
    ;;
  qk_device_wire)
    set -- --no-default-features --features process-s9-wire "$target" "fuzz/corpus/$target"
    ;;
  qk_core_normal_process)
    set -- --no-default-features --features process-s9-core "$target" "fuzz/corpus/$target"
    ;;

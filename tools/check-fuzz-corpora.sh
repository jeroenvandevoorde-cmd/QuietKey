...
sec1210_production_manifest='fuzz/CORPUS-MANIFEST-SEC1210-PRODUCTION-V1.tsv'
sec1210_production_campaign='fuzz/CAMPAIGN-037.md'
sec1210_historical_manifest='fuzz/CORPUS-MANIFEST-SEC1210-R2A.tsv'
sec1210_production_wire_target='fuzz/fuzz_targets/qk_sec1210_wire.rs'
...
# QK-DEC-169 independent pre-campaign renderer; historical evidence remains frozen.
if [ "$mode" = render_sec1210_production ]; then
  sec1210_production_entries=$(mktemp) || fail 'mktemp failed for production SEC1210 entries'
  sec1210_production_expected=$(mktemp) || fail 'mktemp failed for production SEC1210 manifest'
  target_tmp=$(mktemp) || fail 'mktemp failed for production SEC1210 target entries'
  trap 'rm -f "$sec1210_production_entries" "$sec1210_production_expected" "$target_tmp"' EXIT HUP INT TERM
  emit_partition_entries "$sec1210_production_targets" "$sec1210_production_entries"
  render_partition 'QK-SEC1210-PRODUCTION-V1-CORPUS-MANIFEST-V1' "$render_source" \
...
# QK-DEC-167-SUP-003 independent pre-campaign renderer; old partitions are untouched.
if [ "$mode" = render_t1_r2 ]; then
  t1_r2_entries=$(mktemp) || fail 'mktemp failed for T=1 entries'
  t1_r2_expected=$(mktemp) || fail 'mktemp failed for T=1 manifest'
  target_tmp=$(mktemp) || fail 'mktemp failed for T=1 target entries'
  trap 'rm -f "$t1_r2_entries" "$t1_r2_expected" "$target_tmp"' EXIT HUP INT TERM
  emit_partition_entries "$t1_r2_targets" "$t1_r2_entries"
  render_partition 'QK-T1-R2-CORPUS-MANIFEST-V1' "$render_source" \
    "$t1_r2_targets" "$t1_r2_order" "$t1_r2_entries" \
    "$t1_r2_expected"
  sed -n 'p' "$t1_r2_expected"
  exit 0
fi
...
if [ "$mode" = check ]; then
  ...
  sec1210_production_tracked=0
  for target_path in "$sec1210_production_wire_target" "$sec1210_production_core_target"; do
    if git ls-files --error-unmatch -- "$target_path" >/dev/null 2>&1; then
      sec1210_production_tracked=$((sec1210_production_tracked + 1))
      [ -f "$target_path" ] || fail "$target_path is missing or is not a regular file"
      [ ! -L "$target_path" ] || fail "$target_path must not be a symlink"
    fi
  done
  if [ "$sec1210_production_tracked" = 2 ]; then
    [ -f "$sec1210_historical_manifest" ] || \
      fail "$sec1210_historical_manifest is not a regular file"
    [ ! -L "$sec1210_historical_manifest" ] || \
      fail "$sec1210_historical_manifest must not be a symlink"
    git ls-files --error-unmatch -- "$sec1210_historical_manifest" >/dev/null 2>&1 || \
      fail "$sec1210_historical_manifest is untracked"
    [ -f "$sec1210_production_campaign" ] || \
      fail "$sec1210_production_campaign is not a regular file"
    [ ! -L "$sec1210_production_campaign" ] || \
      fail "$sec1210_production_campaign must not be a symlink"
    git ls-files --error-unmatch -- "$sec1210_production_campaign" >/dev/null 2>&1 || \
      fail "$sec1210_production_campaign is untracked"
    sec1210_production_active=yes
    if [ -e "$sec1210_production_manifest" ] || [ -L "$sec1210_production_manifest" ]; then
      [ -f "$sec1210_production_manifest" ] || \
        fail "$sec1210_production_manifest is not a regular file"
      [ ! -L "$sec1210_production_manifest" ] || \
        fail "$sec1210_production_manifest must not be a symlink"
      git ls-files --error-unmatch -- "$sec1210_production_manifest" >/dev/null 2>&1 || \
        fail "$sec1210_production_manifest is untracked"
      grep -Fqx 'Status: EXECUTED — QUALIFYING RUN COMPLETE.' \
        "$sec1210_production_campaign" || \
        fail 'registered production SEC1210 corpus requires completed campaign status'
      sec1210_production_registered=yes
    else
      grep -Fqx 'Status: PLANNED — NOT EXECUTED.' "$sec1210_production_campaign" || \
        fail 'production SEC1210 manifest absent without planned campaign status'
    fi
  elif [ "$sec1210_production_tracked" != 0 ] || \
       git ls-files --error-unmatch -- "$sec1210_production_campaign" >/dev/null 2>&1 || \
       git ls-files --error-unmatch -- "$sec1210_production_manifest" >/dev/null 2>&1 || \
       [ -e "$sec1210_production_core_target" ] || \
       [ -L "$sec1210_production_core_target" ] || \
       [ -e "$sec1210_production_campaign" ] || [ -L "$sec1210_production_campaign" ] || \
  ...
  if git ls-files --error-unmatch -- "$t1_r2_target" >/dev/null 2>&1; then
    ...
    t1_r2_active=yes
    if [ -e "$t1_r2_manifest" ] || [ -L "$t1_r2_manifest" ]; then
      [ -f "$t1_r2_manifest" ] && [ ! -L "$t1_r2_manifest" ] || \
        fail "$t1_r2_manifest is not a regular non-symlink file"
      git ls-files --error-unmatch -- "$t1_r2_manifest" >/dev/null 2>&1 || \
        fail "$t1_r2_manifest is untracked"
      grep -Fqx 'Status: EXECUTED — QUALIFYING RUN COMPLETE.' "$t1_r2_campaign" || \
        fail 'registered T=1 corpus requires completed campaign status'
      t1_r2_registered=yes
    else
      grep -Fqx 'Status: PLANNED — NOT EXECUTED.' "$t1_r2_campaign" || \
        fail 'T=1 manifest absent without planned campaign status'
    fi
  elif [ -e "$t1_r2_campaign" ] || [ -L "$t1_r2_campaign" ] || \
       [ -e "$t1_r2_manifest" ] || [ -L "$t1_r2_manifest" ]; then
    fail 'T=1 target or evidence set is incomplete'
  fi
...
if [ "$sec1210_production_active" = yes ]; then
  all_targets="$all_targets $sec1210_production_targets"
else
  for target in $sec1210_production_targets; do
    if [ -e "fuzz/corpus/$target" ] || [ -L "fuzz/corpus/$target" ] || \
       [ -e "fuzz/findings/$target" ] || [ -L "fuzz/findings/$target" ]; then
      fail "unregistered production SEC1210 corpus or finding root: $target"
    fi
  done
fi
...
if [ "$t1_r2_active" = yes ]; then
  ...
  for target in $t1_r2_targets; do
    if [ -e "fuzz/corpus/$target" ] || [ -L "fuzz/corpus/$target" ] || \
       [ -e "fuzz/findings/$target" ] || [ -L "fuzz/findings/$target" ]; then
      fail "unregistered T=1 corpus or finding root: $target"
    fi
  done
fi
...
sec1210_production_entries=$(mktemp) || fail 'mktemp failed for production SEC1210 entries'
sec1210_production_expected=$(mktemp) || fail 'mktemp failed for production SEC1210 manifest'
sec1210_production_paths=$(mktemp) || fail 'mktemp failed for production SEC1210 paths'
...
manifest_t1_r2_paths=$(mktemp) || fail 'mktemp failed for T=1 manifest paths'
...
if [ "$sec1210_production_active" = yes ]; then
  emit_partition_entries "$sec1210_production_targets" "$sec1210_production_entries"
  awk -F '\t' '$1 == "corpus" && $2 == "qk_sec1210_wire" { print }' \
    "$sec1210_historical_manifest" > "$sec1210_historical_entries" || \
    fail 'cannot extract historical SEC1210 corpus entries'
  historical_count=$(wc -l < "$sec1210_historical_entries" | tr -d ' ')
  historical_bytes=$(awk -F '\t' '{ sum += $3 } END { print sum + 0 }' \
    "$sec1210_historical_entries")
  historical_hash=$(sha256_file "$sec1210_historical_entries") || \
    fail 'cannot hash historical SEC1210 corpus entries'
  [ "$historical_count:$historical_bytes:$historical_hash" = \
    '565:43252:37e77b5c3de454a819ee1e4acb999be21630e8543d9b1b06f6aef02b4689df92' ] || \
    fail 'historical SEC1210 corpus manifest identity changed'
  awk 'NR == FNR { current[$0] = 1; next } !($0 in current) { exit 1 }' \
    "$sec1210_production_entries" "$sec1210_historical_entries" || \
    fail 'production SEC1210 corpus does not preserve every historical wire entry'
else
...
if [ "$t1_r2_active" = yes ]; then
  ...
  : > "$t1_r2_entries" || fail 'cannot initialize T=1 entries'
fi
...
if [ "$sec1210_production_active" = yes ] && \
   [ "$sec1210_production_registered" = no ]; then
  planned_count=$(wc -l < "$sec1210_production_entries" | tr -d ' ')
  planned_bytes=$(awk -F '\t' '{ sum += $3 } END { print sum + 0 }' \
    "$sec1210_production_entries")
  planned_hash=$(sha256_file "$sec1210_production_entries") || \
    fail 'cannot hash production SEC1210 starting entries'
  [ "$planned_count:$planned_bytes:$planned_hash" = \
...
if [ "$t1_r2_active" = yes ] && [ "$t1_r2_registered" = no ]; then
  ...
  planned_hash=$(sha256_file "$t1_r2_entries") || fail 'cannot hash T=1 starting entries'
  [ "$planned_count:$planned_bytes:$planned_hash" = '2:28:3276696359db695227aeaefb9406cd79dfefe9186332ee7fc3fab1f882d03de8' ] || \
    fail 'T=1 starting corpus differs from its preregistered identity'
fi

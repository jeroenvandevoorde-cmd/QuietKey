#!/bin/sh
# Recompute and verify the partitioned QK-DEC-106/QK-DEC-109..113/QK-DEC-116/QK-DEC-118..129 corpus registries.
set -u
...
all_targets="$m21_targets $m22_targets $m23_targets $m24_targets $m25_targets $m26_targets $m27_targets $m28_targets $m29_targets $m30_targets $v2s4_targets $v2s5_targets $v2s6_targets $v2s7_targets"
...
if [ "$mode" = check ]; then
  for manifest in "$m21_manifest" "$m22_manifest" "$m23_manifest" "$m24_manifest" \
    ...
    "$v2s6_manifest" "$v2s7_manifest"; do
...
trap 'rm -f "$m21_entries" "$m22_entries" "$m21_expected" "$m22_expected" \
  ...
  "$v2s4_paths" "$v2s5_paths" "$v2s6_paths" "$v2s7_paths" \
  ...
  "$manifest_v2s6_paths" "$manifest_v2s7_paths"' EXIT HUP INT TERM

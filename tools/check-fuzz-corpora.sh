...
sec1210_r2a_campaign='fuzz/CAMPAIGN-033.md'
...
t1_r2_campaign='fuzz/CAMPAIGN-033.md'
...
unexpected=$(find fuzz/corpus -mindepth 1 -maxdepth 1 \
  ! -name qk_t1 ! -name qk_sec1210_wire ! -name qk_psbt ! -name qk_descriptor ! -name qk_a1 \
...
trap 'rm -f "$t1_r2_entries" "$t1_r2_expected" "$t1_r2_paths" "$manifest_t1_r2_paths" "$sec1210_r2a_entries" "$sec1210_r2a_expected" "$sec1210_r2a_paths" "$manifest_sec1210_r2a_paths" "$m21_entries" "$m22_entries" "$m21_expected" "$m22_expected" \
...
cat "$t1_r2_paths" "$sec1210_r2a_paths" "$m21_paths" "$m22_paths" "$m23_paths" "$m24_paths" "$m25_paths" \
  "$m26_paths" "$m27_paths" "$m28_paths" "$m29_paths" "$m30_paths" \
  "$v2s4_paths" "$v2s5_paths" "$v2s6_paths" "$v2s7_paths" "$v2s8_paths" \
  "$v2s9_paths" "$v2s10_paths" "$v2s11_paths" "$firmware_paths" \
  "$process_s1_paths" "$process_s2_paths" "$process_s3_paths" \

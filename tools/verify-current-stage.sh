...
F32L_C=1408fa98961e62ba6316515b1e277b6952a96e18 # published F3.2m source base / F3.2l commit C
F32M_A=59008528885259ad8b93be1f0ece136930fd445d # docs: authorize F3.2m Q-002 decision packet
F32M_B=96b76dc5a88c7e2430605ad78ed99d8f7a17a6c1 # docs(f3): add F3.2m Q-002 decision packet
F32M_C=f41e26799d0c4522b91e1de72d27cf473116b3ab # published F3.2n source base / F3.2m commit C
F32N_A=d3a4f3bec5902d2e2155647a27ffe34bfa233bee # docs: authorize F3.2n Q-002 owner direction
F32N_B=15b79a3ac61d55ed364b7b991effa00d9d147fdc # docs(f3): record F3.2n Q-002 owner direction
...
# F32L_A -> F32L_B -> F32L_C -> F32M_A -> F32M_B -> F32M_C ->
# F32N_A -> F32N_B -> HEAD, no merges. F32M_C is the unchanged
# published F3.2n source base. F3.2m and every earlier stage remain
...
p_f32n_b=$($GIT rev-parse "$F32N_B^" 2>/dev/null) || err "F32N_B has no parent"
p_f32n_a=$($GIT rev-parse "$F32N_A^" 2>/dev/null) || err "F32N_A has no parent"
p_f32m_c=$($GIT rev-parse "$F32M_C^" 2>/dev/null) || err "F32M_C has no parent"
p_f32m_b=$($GIT rev-parse "$F32M_B^" 2>/dev/null) || err "F32M_B has no parent"
p_f32m_a=$($GIT rev-parse "$F32M_A^" 2>/dev/null) || err "F32M_A has no parent"
p_f32l_c=$($GIT rev-parse "$F32L_C^" 2>/dev/null) || err "F32L_C has no parent"
...
[ "$p_head" = "$F32N_B" ] || err "HEAD parent is $p_head, expected $F32N_B"
[ "$p_f32n_b" = "$F32N_A" ] || err "F32N_B parent is $p_f32n_b, expected $F32N_A"
[ "$p_f32n_a" = "$F32M_C" ] || err "F32N_A parent is $p_f32n_a, expected $F32M_C"
[ "$p_f32m_c" = "$F32M_B" ] || err "F32M_C parent is $p_f32m_c, expected $F32M_B"
[ "$p_f32m_b" = "$F32M_A" ] || err "F32M_B parent is $p_f32m_b, expected $F32M_A"
[ "$p_f32m_a" = "$F32L_C" ] || err "F32M_A parent is $p_f32m_a, expected $F32L_C"
[ "$p_f32l_c" = "$F32L_B" ] || err "F32L_C parent is $p_f32l_c, expected $F32L_B"
...
[ "$count" = "67" ] || err "expected exactly 67 commits after base, found $count"
...
for c in "$head" "$F32N_B" "$F32N_A" "$F32M_C" "$F32M_B" "$F32M_A" "$F32L_C" "$F32K_B" "$F32K_A" "$F32J_C" "$F32J_B" "$F32J_A" "$F32I_C" "$F32I_B" "$F32I_A" "$F32H_C" "$C47" "$C46" "$F32G_C" "$C45" "$C44" "$C43" "$C42" "$C41" "$C40" "$C39" "$C38" "$C37" "$C36" "$C35" "$C34" "$C33" "$C32" "$C31" "$C30" "$C29" "$C28" "$C27" "$C26" "$C25" "$C24" "$C23" "$C22" "$C21" "$C20" "$C19" "$C18" "$C17" "$C16" "$C15" "$C14" "$C13" "$C12" "$C11" "$C10" "$C9" "$C8" "$C7" "$C6" "$C5" "$C4" "$C3" "$C2" "$C1"; do
...
check_message "$F32L_C" "F3.2l commit C" 'chore(verify): bind F3.2l QK-LIM-PSBT-027 canonical amendment stage'
check_message "$F32M_A" "F3.2m commit A" 'docs: authorize F3.2m D-11 Q-002 SD attachment-epoch decision packet'
check_message "$F32M_B" "F3.2m commit B" 'docs(f3): add non-binding F3.2m D-11 Q-002 SD attachment-epoch decision packet'
check_message "$F32M_C" "F3.2m commit C" 'chore(verify): bind F3.2m D-11 Q-002 decision-packet stage'
check_message "$F32N_A" "F3.2n commit A" 'docs: authorize F3.2n D-11 Q-002 physical SD attachment-epoch owner direction'
check_message "$F32N_B" "F3.2n commit B" 'docs(f3): record non-enacting F3.2n D-11 Q-002 physical SD attachment-epoch direction'
check_message "$head" "F3.2n commit C" 'chore(verify): bind F3.2n D-11 Q-002 physical SD attachment-epoch owner-direction stage'
...
check_paths "$F32H_C" "f32h-commit-c" <<'EOF'
tools/verify-current-stage.sh
EOF

check_paths "$F32I_A" "f32i-commit-a" <<'EOF'
docs/DECISION-LOG.md
EOF

check_paths "$F32I_B" "f32i-commit-b" <<'EOF'
...
check_paths "$F32J_C" "f32j-commit-c" <<'EOF'
tools/verify-current-stage.sh
EOF

check_paths "$F32K_A" "f32k-commit-a" <<'EOF'
docs/DECISION-LOG.md
EOF

check_paths "$F32K_B" "f32k-commit-b" <<'EOF'
...
check_paths "$F32L_C" "f32l-commit-c" <<'EOF'
tools/verify-current-stage.sh
EOF

check_paths "$F32M_A" "f32m-commit-a" <<'EOF'
docs/DECISION-LOG.md
EOF

check_paths "$F32M_B" "f32m-commit-b" <<'EOF'
docs/f3/F3.2M-D11-Q002-SD-ATTACHMENT-EPOCH-DECISION-PACKET.md
tools/verify-host-boundary.sh
EOF

check_paths "$F32M_C" "f32m-commit-c" <<'EOF'
tools/verify-current-stage.sh
EOF

check_paths "$F32N_A" "f32n-commit-a" <<'EOF'
docs/DECISION-LOG.md
EOF

check_paths "$F32N_B" "f32n-commit-b" <<'EOF'
docs/f3/F3.2N-D11-Q002-SD-ATTACHMENT-EPOCH-OWNER-DIRECTION-RECORD.md
tools/verify-host-boundary.sh
EOF

check_paths "$head" "f32n-commit-c" <<'EOF'
tools/verify-current-stage.sh
EOF
...
for f32hmodepair in \
  ...
  "$F32H_C 100755 tools/verify-current-stage.sh"; do
...
# F3.2i exact 1/2/1 path-mode matrix above the published F32H_C
# source base. Path sets are checked independently above.
for f32imodepair in \
  ...
  f32imodepath=${f32imoderest#* }
  $GIT ls-tree "$f32imodecommit" -- "$f32imodepath" > "$tmpdir/cs.f32i.mode.raw" \
    || err "F3.2i ls-tree failed for $f32imodepath"
  [ -s "$tmpdir/cs.f32i.mode.raw" ] || err "F3.2i mode entry missing for $f32imodepath"
  f32imodelines=$(awk 'END {print NR+0}' "$tmpdir/cs.f32i.mode.raw") \
    || err "F3.2i mode line count failed for $f32imodepath"
  [ "$f32imodelines" = 1 ] || err "F3.2i mode entry is not unique for $f32imodepath"
  f32imodeact=$(awk 'NR == 1 {print $1}' "$tmpdir/cs.f32i.mode.raw") \
...
for f32jmodepair in \
  ...
  "$F32J_C 100755 tools/verify-current-stage.sh"; do
...
# F3.2k exact 1/2/1 path-mode matrix above the frozen F32J_C source base.
for f32kmodepair in \
  "$F32K_A 100644 docs/DECISION-LOG.md" \
  "$F32K_B 100644 docs/f3/F3.2K-QK-LIM-PSBT-027-EXACT-CONSTRUCTION-OWNER-DIRECTION-RECORD.md" \
  "$F32K_B 100755 tools/verify-host-boundary.sh" \
  "$head 100755 tools/verify-current-stage.sh"; do
  f32kmodecommit=${f32kmodepair%% *}
  f32kmoderest=${f32kmodepair#* }
  f32kmodeexp=${f32kmoderest%% *}
  f32kmodepath=${f32kmoderest#* }
  $GIT ls-tree "$f32kmodecommit" -- "$f32kmodepath" > "$tmpdir/cs.f32k.mode.raw" \
    || err "F3.2k ls-tree failed for $f32kmodepath"
  [ -s "$tmpdir/cs.f32k.mode.raw" ] || err "F3.2k mode entry missing for $f32kmodepath"
  f32kmodelines=$(awk 'END {print NR+0}' "$tmpdir/cs.f32k.mode.raw") \
    || err "F3.2k mode line count failed for $f32kmodepath"
  [ "$f32kmodelines" = 1 ] || err "F3.2k mode entry is not unique for $f32kmodepath"
  f32kmodeact=$(awk 'NR == 1 {print $1}' "$tmpdir/cs.f32k.mode.raw") \
    || err "F3.2k mode extraction failed for $f32kmodepath"
  [ "$f32kmodeact" = "$f32kmodeexp" ] \
    || err "F3.2k mode for $f32kmodepath is $f32kmodeact, expected $f32kmodeexp"
done
...
grep -F 'F3.2l published Decision Log differs from the reviewed A' tools/verify-host-boundary.sh >/dev/null \
...
# This is the frozen published F3.2h stage.
...
$GIT diff --name-only "$F32G_C" "$F32H_C" > "$tmpdir/cs.f32h.paths.raw"
csf32hrc=$?
[ "$csf32hrc" -eq 0 ] || err "F3.2h F32G_C..F32H_C path enumeration failed"
[ -s "$tmpdir/cs.f32h.paths.raw" ] || err "F3.2h F32G_C..F32H_C path enumeration is empty"
LC_ALL=C sort "$tmpdir/cs.f32h.paths.raw" > "$tmpdir/cs.f32h.paths.act"
csf32hrc=$?
[ "$csf32hrc" -eq 0 ] || err "F3.2h F32G_C..F32H_C path sort failed"
cmp -s "$tmpdir/cs.f32h.paths.exp" "$tmpdir/cs.f32h.paths.act"
csf32hrc=$?
[ "$csf32hrc" -eq 0 ] || err "F3.2h F32G_C..F32H_C path union is not the exact authorized four-path set"

# Commit A is one append-only Decision Log update preserved exactly at the
# published F3.2h C stage. Commit B packet remains active; its host
# bytes remain exact in that frozen F3.2h C snapshot.
...
$GIT show "$F32H_C:docs/DECISION-LOG.md" > "$tmpdir/cs.f32h.dlog.stage" 2>/dev/null
csf32hrc=$?
[ "$csf32hrc" -eq 0 ] || err "cannot read F3.2h C Decision Log snapshot"
cmp -s "$tmpdir/cs.f32h.dlog.a" "$tmpdir/cs.f32h.dlog.stage"
csf32hrc=$?
[ "$csf32hrc" -eq 0 ] || err "F3.2h C Decision Log snapshot is not byte-identical to F3.2h A"
...
for csf32htreepair in \
  ...
  $GIT rev-parse "$F32H_C:$csf32htreepath" > "$tmpdir/cs.f32h.stage.blob"
  csf32hrc=$?
  [ "$csf32hrc" -eq 0 ] || err "F3.2h C snapshot blob scan failed for $csf32htreepath"
  printf '%s\n' "$csf32htreeblob" > "$tmpdir/cs.f32h.stage.exp" \
    || err "F3.2h C snapshot blob fixture failed"
  cmp -s "$tmpdir/cs.f32h.stage.exp" "$tmpdir/cs.f32h.stage.blob"
  csf32hrc=$?
  [ "$csf32hrc" -eq 0 ] || err "F3.2h C snapshot blob differs for $csf32htreepath"
done
...
$GIT show "$F32H_C:tools/verify-host-boundary.sh" > "$tmpdir/cs.f32h.host.stage" 2>/dev/null
csf32hrc=$?
[ "$csf32hrc" -eq 0 ] || err "cannot read F3.2h host verifier from F32H_C"
cmp -s "$tmpdir/cs.f32h.host.b" "$tmpdir/cs.f32h.host.stage"
csf32hrc=$?
[ "$csf32hrc" -eq 0 ] || err "F3.2h C host snapshot is not byte-identical to C47/B"
...
# The frozen F3.2h C host blob is pinned above. Bind its operative guards
# independently before invoking the active host verifier below.
for csf32hhostfixture in \
  ...
  csf32hhostcount=$(grep -cF -e "$csf32hhostfixture" "$tmpdir/cs.f32h.host.stage")
  csf32hrc=$?
  [ "$csf32hrc" -le 1 ] || err "historical F3.2h C host mechanism scan failed"
...
tr -d '[:space:]' < "$tmpdir/cs.f32i.dlog.bytes.raw" > "$tmpdir/cs.f32i.dlog.bytes"
csf32irc=$?
[ "$csf32irc" -eq 0 ] || err "F3.2i Decision Log byte-count normalization failed"
csf32ibasebytes=$(cat "$tmpdir/cs.f32i.dlog.bytes")
csf32irc=$?
[ "$csf32irc" -eq 0 ] || err "F3.2i Decision Log byte-count read failed"
case $csf32ibasebytes in ''|*[!0-9]*) err "F3.2i Decision Log byte count is non-numeric" ;; esac
dd if="$tmpdir/cs.f32i.dlog.a" bs=1 count="$csf32ibasebytes" \
  > "$tmpdir/cs.f32i.dlog.prefix" 2> "$tmpdir/cs.f32i.dlog.prefix.err"
csf32irc=$?
[ "$csf32irc" -eq 0 ] || err "F3.2i Decision Log prefix read failed"
cmp -s "$tmpdir/cs.f32i.dlog.base" "$tmpdir/cs.f32i.dlog.prefix"
csf32irc=$?
[ "$csf32irc" -eq 0 ] || err "F3.2i base Decision Log is not the exact prefix of A"

for csf32itreepair in \
  "$F32I_A 100644 a5608d74ff3f5ec5b5b1d99723d481c510442c86 docs/DECISION-LOG.md" \
  "$F32I_B 100644 2f4bc0f5cb8e861b45f515fb7c4c5dbc764784c5 $F32IREC" \
  "$F32I_B 100755 f011310f42298dead3a6fafb4e0a103b4c0c62e9 tools/verify-host-boundary.sh"; do
  csf32itreecommit=${csf32itreepair%% *}
  csf32itreerest=${csf32itreepair#* }
  csf32itreemode=${csf32itreerest%% *}
  csf32itreerest=${csf32itreerest#* }
  csf32itreeblob=${csf32itreerest%% *}
  csf32itreepath=${csf32itreerest#* }
  $GIT ls-tree "$csf32itreecommit" -- "$csf32itreepath" > "$tmpdir/cs.f32i.tree.raw"
  csf32irc=$?
  [ "$csf32irc" -eq 0 ] || err "F3.2i tree lookup failed for $csf32itreepath"
  [ -s "$tmpdir/cs.f32i.tree.raw" ] || err "F3.2i tree entry missing for $csf32itreepath"
  csf32itreelines=$(awk 'END {print NR+0}' "$tmpdir/cs.f32i.tree.raw")
  csf32irc=$?
  [ "$csf32irc" -eq 0 ] || err "F3.2i tree line count failed for $csf32itreepath"
  [ "$csf32itreelines" = 1 ] || err "F3.2i tree entry is not unique for $csf32itreepath"
  csf32itreeactmode=$(awk 'NR == 1 {print $1}' "$tmpdir/cs.f32i.tree.raw")
  csf32irc=$?
  [ "$csf32irc" -eq 0 ] || err "F3.2i tree mode extraction failed for $csf32itreepath"
  csf32itreeactblob=$(awk 'NR == 1 {print $3}' "$tmpdir/cs.f32i.tree.raw")
  csf32irc=$?
...
while IFS=' ' read -r csf32iprotected csf32iprotectedblob; do
  ...
  csf32irc=$?
  [ "$csf32irc" -eq 0 ] || err "F3.2i protected blob differs for $csf32iprotected"
done <<'QK_CS_F32I_PROTECTED_EOF'
docs/RESOURCE-BUDGETS.md 1ae6494fc24a8e3572464cf45e6d43ea4875e95d
docs/REQUIREMENTS.md 981b1b497102187da66fb4e82ef0725b32c088f7
docs/TEST-ARCHITECTURE.md 065e20eed4e916c907a244aa3e5ca2e66cbc5d4e
docs/f3/F3.2E-PSBT-OUTPUT-TEST-ALIGNMENT-PACKET.md 4ffa20afbedeb0b6cfbbe57298f941bc0537683e
...
QK_CS_F32I_PACKET_OPTIONS_EOF
grep '^| F32H-LIM-OPT-' "$tmpdir/cs.f32i.packet.stage" > "$tmpdir/cs.f32i.packet.options.act"
csf32irc=$?
[ "$csf32irc" -eq 0 ] || err "F3.2i historical packet option extraction failed"
cmp -s "$tmpdir/cs.f32i.packet.options.exp" "$tmpdir/cs.f32i.packet.options.act"
csf32irc=$?
...
for csf32iowner in \
  ...
  for csf32iownerfile in "$tmpdir/cs.f32i.dlog.stage" "$tmpdir/cs.f32i.record.stage"; do
    ...
    [ "$csf32iownercount" = 1 ] || err "F3.2i owner transcript differs or duplicates in $csf32iownerfile"
  done
done
...
csf32irc=$?
[ "$csf32irc" -eq 0 ] || err "F3.2i record owner block deframing failed"
cmp -s "$tmpdir/cs.f32i.owner.dlog" "$tmpdir/cs.f32i.owner.record"
csf32irc=$?
...
QK_CS_F32I_RECORD_OPTIONS_EOF
cmp -s "$tmpdir/cs.f32i.record.options.exp" "$tmpdir/cs.f32i.record.options.act"
csf32irc=$?
...
for csf32ihostfixture in \
  ...
  'cmp -s "$tmpdir/p32i.options.exp" "$tmpdir/p32i.options.act"' \
  'selection grammar differs' \
  'historical source state at packet publication' \
  'canonical live-link cell differs' \
  'whole-record bytes differ' \
...
# -------- F3.2j QK-LIM-PSBT-027 exact construction historical stage
# This frozen published stage remains non-binding and non-enacting.
# Its sole candidate remains historically PROPOSED — UNSELECTED.
F32JPKT=docs/f3/F3.2J-QK-LIM-PSBT-027-EXACT-TOMBSTONE-CONSTRUCTION-PACKET.md
[ -f "$F32JPKT" ] || err "$F32JPKT missing"

ahead=$($GIT rev-list --count "$F32I_C..$F32J_C")
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "F3.2j frozen-base ahead count failed"
behind=$($GIT rev-list --count "$F32J_C..$F32I_C")
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "F3.2j frozen-base behind count failed"
[ "$ahead" = 3 ] || err "F3.2j frozen stage is $ahead commits above its base, expected exactly 3"
[ "$behind" = 0 ] || err "F3.2j base is $behind commits ahead of frozen stage, expected exactly 0"
...
$GIT diff --name-only "$F32I_C" "$F32J_C" > "$tmpdir/cs.f32j.paths.raw"
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "F3.2j F32I_C..F32J_C path enumeration failed"
[ -s "$tmpdir/cs.f32j.paths.raw" ] || err "F3.2j F32I_C..F32J_C path enumeration is empty"
LC_ALL=C sort "$tmpdir/cs.f32j.paths.raw" > "$tmpdir/cs.f32j.paths.act"
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "F3.2j F32I_C..F32J_C path sort failed"
cmp -s "$tmpdir/cs.f32j.paths.exp" "$tmpdir/cs.f32j.paths.act"
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "F3.2j F32I_C..F32J_C path union is not the exact authorized four-path set"

# Commit A is one exact append-only Decision Log update in the frozen stage.
# Commit B packet and host bytes remain exact at frozen F32J_C.
...
$GIT show "$F32J_C:docs/DECISION-LOG.md" > "$tmpdir/cs.f32j.dlog.frozen" 2>/dev/null
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "cannot read frozen F3.2j Decision Log"
cmp -s "$tmpdir/cs.f32j.dlog.a" "$tmpdir/cs.f32j.dlog.frozen"
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "frozen F3.2j Decision Log is not byte-identical to A"
...
for csf32jtreepair in \
  ...
  $GIT rev-parse "$F32J_C:$csf32jtreepath" > "$tmpdir/cs.f32j.frozen.blob" 2>/dev/null
  csf32jrc=$?
  [ "$csf32jrc" -eq 0 ] || err "F3.2j frozen blob lookup failed for $csf32jtreepath"
  printf '%s\n' "$csf32jtreeblob" > "$tmpdir/cs.f32j.frozen.exp" \
    || err "F3.2j frozen blob fixture failed"
  cmp -s "$tmpdir/cs.f32j.frozen.exp" "$tmpdir/cs.f32j.frozen.blob"
  csf32jrc=$?
  [ "$csf32jrc" -eq 0 ] || err "F3.2j frozen blob differs for $csf32jtreepath"
done
$GIT show "$F32J_B:$F32JPKT" > "$tmpdir/cs.f32j.packet.b" 2>/dev/null
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "cannot read F3.2j packet from B"
$GIT show "$F32J_C:$F32JPKT" > "$tmpdir/cs.f32j.packet.frozen" 2>/dev/null
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "cannot read frozen F3.2j packet"
cmp -s "$tmpdir/cs.f32j.packet.b" "$tmpdir/cs.f32j.packet.frozen"
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "frozen F3.2j packet is not byte-identical to B"
cmp -s "$tmpdir/cs.f32j.packet.frozen" "$F32JPKT"
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "active F3.2j packet differs from frozen packet"
...
$GIT show "$F32J_C:tools/verify-host-boundary.sh" > "$tmpdir/cs.f32j.host.frozen" 2>/dev/null
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "cannot read frozen F3.2j host verifier"
cmp -s "$tmpdir/cs.f32j.host.b" "$tmpdir/cs.f32j.host.frozen"
csf32jrc=$?
[ "$csf32jrc" -eq 0 ] || err "frozen F3.2j host verifier is not byte-identical to B"
...
for csf32jowner in \
  ...
  for csf32jownerfile in "$tmpdir/cs.f32j.dlog.a" "$F32JPKT"; do
...
sed -n '/^### QK-AUTH-F3.2J-QK-LIM-PSBT-027-CONSTRUCT-001 /,$p' \
  "$tmpdir/cs.f32j.dlog.a" > "$tmpdir/cs.f32j.dlog.section"
csf32jrc=$?
...
# Bind the frozen F3.2j host mechanisms independently. Active F3.2k
# mechanisms and strict host invocation are checked below.
for csf32jhostfixture in \
  ...
  csf32jhostcount=$(grep -cF -e "$csf32jhostfixture" "$tmpdir/cs.f32j.host.b")
  csf32jrc=$?
  [ "$csf32jrc" -le 1 ] || err "frozen host F3.2j mechanism scan failed"
...
csf32krc=$?
[ "$csf32krc" -eq 0 ] || err "cannot read F3.2k base Decision Log"
...
tr -d '[:space:]' < "$tmpdir/cs.f32k.dlog.bytes.raw" > "$tmpdir/cs.f32k.dlog.bytes"
csf32krc=$?
[ "$csf32krc" -eq 0 ] || err "F3.2k Decision Log byte-count normalization failed"
csf32kbasebytes=$(cat "$tmpdir/cs.f32k.dlog.bytes")
csf32krc=$?
[ "$csf32krc" -eq 0 ] || err "F3.2k Decision Log byte-count read failed"
case $csf32kbasebytes in ''|*[!0-9]*) err "F3.2k Decision Log byte count is non-numeric" ;; esac
dd if="$tmpdir/cs.f32k.dlog.a" bs=1 count="$csf32kbasebytes" \
  > "$tmpdir/cs.f32k.dlog.prefix" 2> "$tmpdir/cs.f32k.dlog.prefix.err"
csf32krc=$?
[ "$csf32krc" -eq 0 ] || err "F3.2k Decision Log prefix read failed"
cmp -s "$tmpdir/cs.f32k.dlog.base" "$tmpdir/cs.f32k.dlog.prefix"
csf32krc=$?
[ "$csf32krc" -eq 0 ] || err "F3.2k base Decision Log is not the exact prefix of A"

# Independently bind B's exact parent, two-path set, modes, and blobs.
p32kb=17d190a98465411aafba88a186b531728e2c8c9b
p32khostblob=a8254ab4d78800f1714f91967cb461123d9275ad
$GIT rev-parse "$p32kb^{commit}" > "$tmpdir/cs.f32k.b.act" 2>/dev/null
csf32krc=$?
[ "$csf32krc" -eq 0 ] || err "F3.2k B does not resolve"
printf '%s\n' "$p32kb" > "$tmpdir/cs.f32k.b.exp" || err "F3.2k B fixture failed"
cmp -s "$tmpdir/cs.f32k.b.exp" "$tmpdir/cs.f32k.b.act"
csf32krc=$?
[ "$csf32krc" -eq 0 ] || err "F3.2k B differs"
p32kbparent=$($GIT rev-parse "$p32kb^" 2>/dev/null)
csf32krc=$?
[ "$csf32krc" -eq 0 ] || err "F3.2k B parent lookup failed"
[ "$p32kbparent" = "$F32K_A" ] || err "F3.2k B parent differs"
printf '%s\n' "$REC32K" tools/verify-host-boundary.sh > "$tmpdir/cs.f32k.b.paths.exp" || err "F3.2k B path fixture failed"
$GIT diff-tree --no-commit-id --name-only -r "$p32kb" > "$tmpdir/cs.f32k.b.paths.act"
csf32krc=$?
[ "$csf32krc" -eq 0 ] || err "F3.2k B path enumeration failed"
cmp -s "$tmpdir/cs.f32k.b.paths.exp" "$tmpdir/cs.f32k.b.paths.act"
csf32krc=$?
[ "$csf32krc" -eq 0 ] || err "F3.2k B path set differs"
for csf32ktreepair in \
  "$p32kb 100644 5b05d62776c65bfaf5ec41ab39108bc0531b7944 $REC32K" \
  "$p32kb 100755 $p32khostblob tools/verify-host-boundary.sh"; do
  csf32ktreecommit=${csf32ktreepair%% *}
  csf32ktreerest=${csf32ktreepair#* }
  csf32ktreemode=${csf32ktreerest%% *}
  csf32ktreerest=${csf32ktreerest#* }
  csf32ktreeblob=${csf32ktreerest%% *}
  csf32ktreepath=${csf32ktreerest#* }
  $GIT ls-tree "$csf32ktreecommit" -- "$csf32ktreepath" > "$tmpdir/cs.f32k.tree.raw"
  csf32krc=$?
  [ "$csf32krc" -eq 0 ] || err "F3.2k tree lookup failed for $csf32ktreepath"
  [ -s "$tmpdir/cs.f32k.tree.raw" ] || err "F3.2k tree entry missing for $csf32ktreepath"
  csf32ktreeactmode=$(awk 'NR == 1 {print $1}' "$tmpdir/cs.f32k.tree.raw")
  csf32krc=$?
  [ "$csf32krc" -eq 0 ] || err "F3.2k tree mode extraction failed for $csf32ktreepath"
  csf32ktreeactblob=$(awk 'NR == 1 {print $3}' "$tmpdir/cs.f32k.tree.raw")
  csf32krc=$?
  ...
  cmp -s "$tmpdir/cs.f32k.active.exp" "$tmpdir/cs.f32k.active.blob"
  csf32krc=$?
  [ "$csf32krc" -eq 0 ] || err "F3.2k active blob differs for $csf32ktreepath"
done
...
$GIT rev-parse "$p32kbase^{commit}" > "$tmpdir/p32k.base.act" 2>/dev/null
p32krc=$?
...
$GIT rev-parse "$p32ka^{commit}" > "$tmpdir/p32k.a.act" 2>/dev/null
p32krc=$?
...
p32kaparent=$($GIT rev-parse "$p32ka^" 2>/dev/null); p32krc=$?
[ "$p32krc" -eq 0 ] || err "F3.2k A parent lookup failed"
[ "$p32kaparent" = "$p32kbase" ] || err "F3.2k A parent differs"
$GIT diff-tree --no-commit-id --name-only -r "$p32ka" > "$tmpdir/p32k.a.paths.act"; p32krc=$?
...
# The source packet remains historical and keeps its one candidate unselected.
csf32kcandidatehits=$(grep -cFx -e '| F32J-LIM-CON-001 | PROPOSED — UNSELECTED | RECOMMENDED — NOT SELECTED | NONE — FUTURE CONSTRUCTION ONLY |' "$F32JPKT")
csf32krc=$?
...
for csf32khostfixture in \
  ...
  'p32krecordblob=5b05d62776c65bfaf5ec41ab39108bc0531b7944' \
  'p32kpacketblob=b1c2153f5f89a0e1d44d62a09921251e225c0d87' \
  'cmp -s "$tmpdir/p32k.owner.dlog" "$tmpdir/p32k.owner.record"' \
  'one-construction direction map differs' \
  'exact status differs or duplicates' \
  'payload $p32knum differs byte-for-byte from F3.2j' \
  'payload heading cardinality differs' \
  'F3.2k protected blob differs' \
  '[ "$p32krc" -eq 1 ] || err "$REC32K must not end in double LF or cmp failed (exit $p32krc)"' \
  'forbid "$REC32K contains a standalone numeric zero that could be mistaken for a value"' \
  'fourteen-path scan' \
...
# The published F3.2l stage enacted all and only the seven owner-selected canonical payloads.
csf32lbase=$F32K_C
csf32la=$F32L_A
csf32lb=$F32L_B
csf32lc=$F32L_C
...
csf32lprovenancecount=$(grep -cE \
  '^# The published F3[.]2l stage enacted all and only the seven owner-selected canonical payloads[.]$' \
  tools/verify-current-stage.sh)
csf32lrc=$?
[ "$csf32lrc" -le 1 ] || err "F3.2l published provenance comment scan failed"
[ "$csf32lprovenancecount" = 1 ] || err "F3.2l published provenance comment differs or duplicates"
forbid "current-stage verifier restores stale F3.2l local/unpublished provenance wording" \
  -E '^# The published F3[.]2k stage is immutable history[.] This local[/]unpublished$' \
  tools/verify-current-stage.sh
forbid "current-stage verifier restores stale F3.2l enactment wording" \
  -E '^# stage enacts all and only the seven owner-selected canonical payloads[.]$' \
  tools/verify-current-stage.sh
...
$GIT show "$csf32lc:docs/DECISION-LOG.md" > "$tmpdir/cs.f32l.dlog.frozen" 2>/dev/null
csf32lrc=$?
[ "$csf32lrc" -eq 0 ] || err "cannot read frozen F3.2l Decision Log"
cmp -s "$tmpdir/cs.f32l.dlog.a" "$tmpdir/cs.f32l.dlog.frozen"
csf32lrc=$?
[ "$csf32lrc" -eq 0 ] || err "frozen F3.2l Decision Log is not byte-identical to F3.2l A"
...
# F3.2l C is a frozen published source snapshot. F3.2m adds only an
# append-only owner authorization, one non-binding packet, and verifier
# bindings; its exact published C is now a frozen historical stage.
F32MPKT=docs/f3/F3.2M-D11-Q002-SD-ATTACHMENT-EPOCH-DECISION-PACKET.md
csf32mbase=$F32L_C
csf32ma=$F32M_A
csf32mb=$F32M_B
csf32mdlogblob=99384f6cd5eedd37488e0c12d865737217f88b6c
csf32mpacketblob=11714544ea89b3475d65a8a5634b7fdab65f5972
csf32mhostblob=c9e69b00e2b4911bbe19baa15e5132156dcf1781

for csf32mtreepair in \
  "$csf32ma 100644 $csf32mdlogblob docs/DECISION-LOG.md" \
  "$csf32mb 100644 $csf32mpacketblob $F32MPKT" \
  "$csf32mb 100755 $csf32mhostblob tools/verify-host-boundary.sh" \
  "$F32M_C 100755 - tools/verify-current-stage.sh"; do
  csf32mtreecommit=${csf32mtreepair%% *}
  csf32mtreerest=${csf32mtreepair#* }
  csf32mtreemode=${csf32mtreerest%% *}
  csf32mtreerest=${csf32mtreerest#* }
  csf32mtreeblob=${csf32mtreerest%% *}
  csf32mtreepath=${csf32mtreerest#* }
  $GIT ls-tree "$csf32mtreecommit" -- "$csf32mtreepath" > "$tmpdir/cs.f32m.tree"
  csf32mrc=$?
  [ "$csf32mrc" -eq 0 ] || err "F3.2m tree lookup failed for $csf32mtreepath"
  [ -s "$tmpdir/cs.f32m.tree" ] || err "F3.2m tree entry missing for $csf32mtreepath"
  csf32mtreecount=$(awk 'END {print NR+0}' "$tmpdir/cs.f32m.tree")
  csf32mrc=$?
  [ "$csf32mrc" -eq 0 ] || err "F3.2m tree-entry count failed for $csf32mtreepath"
  [ "$csf32mtreecount" = 1 ] || err "F3.2m tree entry is not unique for $csf32mtreepath"
  csf32mtreeactmode=$(awk 'NR == 1 {print $1}' "$tmpdir/cs.f32m.tree")
  csf32mrc=$?
  [ "$csf32mrc" -eq 0 ] || err "F3.2m tree mode extraction failed for $csf32mtreepath"
  csf32mtreeactblob=$(awk 'NR == 1 {print $3}' "$tmpdir/cs.f32m.tree")
  csf32mrc=$?
  [ "$csf32mrc" -eq 0 ] || err "F3.2m tree blob extraction failed for $csf32mtreepath"
  [ "$csf32mtreeactmode" = "$csf32mtreemode" ] || err "F3.2m mode differs for $csf32mtreepath"
  if [ "$csf32mtreeblob" != "-" ]; then
    [ "$csf32mtreeactblob" = "$csf32mtreeblob" ] || err "F3.2m blob differs for $csf32mtreepath"
  fi
done

$GIT diff --name-only "$csf32mbase" "$F32M_C" > "$tmpdir/cs.f32m.paths.raw"
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "F3.2m cumulative path enumeration failed"
LC_ALL=C sort "$tmpdir/cs.f32m.paths.raw" > "$tmpdir/cs.f32m.paths.act"
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "F3.2m cumulative path sort failed"
cat > "$tmpdir/cs.f32m.paths.exp" <<'QK_CS_F32M_PATHS_EOF' || err "F3.2m cumulative path fixture failed"
docs/DECISION-LOG.md
docs/f3/F3.2M-D11-Q002-SD-ATTACHMENT-EPOCH-DECISION-PACKET.md
tools/verify-current-stage.sh
tools/verify-host-boundary.sh
QK_CS_F32M_PATHS_EOF
diff "$tmpdir/cs.f32m.paths.exp" "$tmpdir/cs.f32m.paths.act" > "$tmpdir/cs.f32m.paths.diff" 2>&1
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "F3.2m cumulative path set differs: $(cat "$tmpdir/cs.f32m.paths.diff")"

$GIT show "$csf32ma:docs/DECISION-LOG.md" > "$tmpdir/cs.f32m.dlog.a" 2>/dev/null
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "cannot read F3.2m A Decision Log"
$GIT show "$F32M_C:docs/DECISION-LOG.md" > "$tmpdir/cs.f32m.dlog.c" 2>/dev/null
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "cannot read F3.2m C Decision Log"
cmp -s "$tmpdir/cs.f32m.dlog.a" "$tmpdir/cs.f32m.dlog.c"
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "historical F3.2m C Decision Log differs from exact F3.2m A blob"
$GIT show "$csf32mb:$F32MPKT" > "$tmpdir/cs.f32m.packet.b" 2>/dev/null
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "cannot read F3.2m packet from B"
cmp -s "$tmpdir/cs.f32m.packet.b" "$F32MPKT"
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "active F3.2m packet differs from exact B blob"
...
for csf32msource in \
  ...
  'docs/THREAT-MODEL.md 9bb66dc93d452e04284e13026814ed7322fb25b0'; do
  csf32mpath=${csf32msource%% *}
  csf32mblob=${csf32msource#* }
  $GIT rev-parse "$csf32mbase:$csf32mpath" > "$tmpdir/cs.f32m.source.base" 2>/dev/null
  csf32mrc=$?
  [ "$csf32mrc" -eq 0 ] || err "F3.2m frozen-source lookup failed for $csf32mpath"
  printf '%s\n' "$csf32mblob" > "$tmpdir/cs.f32m.source.exp" || err "F3.2m frozen-source fixture failed"
  cmp -s "$tmpdir/cs.f32m.source.exp" "$tmpdir/cs.f32m.source.base"
  csf32mrc=$?
  [ "$csf32mrc" -eq 0 ] || err "F3.2m frozen-source blob differs for $csf32mpath"
  $GIT rev-parse "$head:$csf32mpath" > "$tmpdir/cs.f32m.source.head" 2>/dev/null
  csf32mrc=$?
  [ "$csf32mrc" -eq 0 ] || err "F3.2m active protected-source lookup failed for $csf32mpath"
  cmp -s "$tmpdir/cs.f32m.source.exp" "$tmpdir/cs.f32m.source.head"
  csf32mrc=$?
  [ "$csf32mrc" -eq 0 ] || err "F3.2m protected source differs for $csf32mpath"
done
...
for csf32mline in \
  ...
  '| F32M-D11-Q002-OPT-001 | PROPOSED — UNSELECTED | RECOMMENDED — NOT SELECTED |' \
  '| F32M-D11-Q002-OPT-002 | PROPOSED — UNSELECTED | NOT RECOMMENDED |' \
  '| F32M-D11-Q002-OPT-003 | PROPOSED — UNSELECTED | NOT RECOMMENDED |' \
  'F32C-D11-Q-002 remains unresolved; neither attachment-epoch binding nor logical-route binding is selected.' \
  '`F32M-D11-Q002-OPT-002` does not reconcile, waive, or amend `QK-REQ-BND-003`.' \
  'F32M-D11-Q002-OPT-003 selects neither rule because it defers F32C-D11-Q-002.' \
  'permits no automatic fallback, retry, or re-signing. No route-output-begins boundary is selected.' \
  ...
  grep -F "$csf32mline" "$tmpdir/cs.f32m.packet.b" > /dev/null 2>&1
  csf32mrc=$?
  [ "$csf32mrc" -eq 0 ] || err "F3.2m packet semantic fixture missing: $csf32mline"
done
for csf32mexactline in \
  ...
  '- The published F3.2d Q-001 clarification remains current and unchanged. Before route output begins, failure releases nothing; after output begins, a later failure stops further output, permits no complete-artifact, delivery, receipt, finalization, broadcast, atomicity, or durability claim, and permits no automatic fallback, retry, or re-signing. No route-output-begins boundary is selected.'; do
  csf32mexacthits=$(grep -cFx -e "$csf32mexactline" "$tmpdir/cs.f32m.packet.b")
  csf32mrc=$?
  [ "$csf32mrc" -le 1 ] || err "F3.2m repaired semantic line scan failed"
  [ "$csf32mexacthits" = 1 ] || err "F3.2m repaired semantic line differs or duplicates"
done
forbid "F3.2m packet reintroduces an overbroad all-dispositions attempt rule" \
  -F 'under any disposition' "$tmpdir/cs.f32m.packet.b"
forbid "F3.2m packet conflates prohibited actions with a retry/re-signing claim" \
  -F 'retry, or re-signing claim' "$tmpdir/cs.f32m.packet.b"
csf32moptions=$(grep -c '^| F32M-D11-Q002-OPT-' "$tmpdir/cs.f32m.packet.b")
csf32mrc=$?
[ "$csf32mrc" -le 1 ] || err "F3.2m option-row count failed"
[ "$csf32moptions" = 3 ] || err "F3.2m packet must contain exactly three option rows"

$GIT show "$csf32mb:tools/verify-host-boundary.sh" > "$tmpdir/cs.f32m.host.b" 2>/dev/null
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "cannot read F3.2m host verifier from B"
$GIT show "$F32M_C:tools/verify-host-boundary.sh" > "$tmpdir/cs.f32m.host.c" 2>/dev/null
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "cannot read F3.2m host verifier from C"
cmp -s "$tmpdir/cs.f32m.host.b" "$tmpdir/cs.f32m.host.c"
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "historical F3.2m C host verifier differs from exact F3.2m B blob"
for csf32mhostline in \
  'p32mpacketblob=11714544ea89b3475d65a8a5634b7fdab65f5972' \
  'F3.2m checked-out branch is not main' \
  'F3.2m HEAD is neither exact intermediate B nor exact final C by bodyless message' \
  'F3.2m B parent differs from exact A' \
  'F3.2m final C parent differs from derived B' \
  'F3.2m B path set differs' \
  'F3.2m C path set differs' \
  'F3.2m intermediate B cumulative path set differs' \
  'F3.2m final C cumulative path set differs' \
  'F3.2m exact three-option content, order, selection, or recommendation differs' \
  'F3.2m protected base blob differs' \
  'F3.2l published C'; do
  grep -F "$csf32mhostline" "$tmpdir/cs.f32m.host.b" > /dev/null 2>&1
  csf32mrc=$?
  [ "$csf32mrc" -eq 0 ] || err "F3.2m host mechanism missing: $csf32mhostline"
done
sh tools/verify-host-boundary.sh > "$tmpdir/cs.f32m.host.out" 2>&1
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "F3.2m strict host verifier failed: $(cat "$tmpdir/cs.f32m.host.out")"
grep -x 'HOST BOOTSTRAP SCOPE PASS' "$tmpdir/cs.f32m.host.out" > /dev/null 2>&1
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "F3.2m host verifier did not emit its sole exact PASS line"
csf32mhostlines=$(awk 'END {print NR+0}' "$tmpdir/cs.f32m.host.out")
csf32mrc=$?
[ "$csf32mrc" -eq 0 ] || err "F3.2m host verifier output count failed"
[ "$csf32mhostlines" = 1 ] || err "F3.2m host verifier emitted extra output"

# -------- F3.2n D-11 Q-002 physical SD attachment-epoch owner direction
# F32M_C is the exact published source base. A and B are pinned above;
# HEAD is accepted only as C by exact parent, message, path, mode, and
# 67-commit ancestry. This verifier never pins its own C blob.
F32NREC=docs/f3/F3.2N-D11-Q002-SD-ATTACHMENT-EPOCH-OWNER-DIRECTION-RECORD.md
csf32nbase=$F32M_C
csf32na=$F32N_A
csf32nb=$F32N_B
csf32ndlogbaseblob=99384f6cd5eedd37488e0c12d865737217f88b6c
csf32ndlogablob=84f8fd26b311fe3e084b01c932ae3ee7469c49a3
csf32nrecordblob=01f7fb3140bcf69c31719a996c7a4c7ddd2394ab
csf32nhostblob=2b4e8946dea626b08fe9b5149b448d7673e4bcc3
csf32npacketblob=11714544ea89b3475d65a8a5634b7fdab65f5972

for csf32ntreepair in \
  "$csf32na 100644 $csf32ndlogablob docs/DECISION-LOG.md" \
  "$csf32nb 100644 $csf32nrecordblob $F32NREC" \
  "$csf32nb 100755 $csf32nhostblob tools/verify-host-boundary.sh" \
  "$head 100755 - tools/verify-current-stage.sh"; do
  csf32ntreecommit=${csf32ntreepair%% *}
  csf32ntreerest=${csf32ntreepair#* }
  csf32ntreemode=${csf32ntreerest%% *}
  csf32ntreerest=${csf32ntreerest#* }
  csf32ntreeblob=${csf32ntreerest%% *}
  csf32ntreepath=${csf32ntreerest#* }
  $GIT ls-tree "$csf32ntreecommit" -- "$csf32ntreepath" > "$tmpdir/cs.f32n.tree"
  csf32nrc=$?
  [ "$csf32nrc" -eq 0 ] || err "F3.2n tree lookup failed for $csf32ntreepath"
  [ -s "$tmpdir/cs.f32n.tree" ] || err "F3.2n tree entry missing for $csf32ntreepath"
  csf32ntreecount=$(awk 'END {print NR+0}' "$tmpdir/cs.f32n.tree")
  csf32nrc=$?
  [ "$csf32nrc" -eq 0 ] || err "F3.2n tree-entry count failed for $csf32ntreepath"
  [ "$csf32ntreecount" = 1 ] || err "F3.2n tree entry is not unique for $csf32ntreepath"
  csf32ntreeactmode=$(awk 'NR == 1 {print $1}' "$tmpdir/cs.f32n.tree")
  csf32nrc=$?
  [ "$csf32nrc" -eq 0 ] || err "F3.2n tree mode extraction failed for $csf32ntreepath"
  csf32ntreeactblob=$(awk 'NR == 1 {print $3}' "$tmpdir/cs.f32n.tree")
  csf32nrc=$?
  [ "$csf32nrc" -eq 0 ] || err "F3.2n tree blob extraction failed for $csf32ntreepath"
  [ "$csf32ntreeactmode" = "$csf32ntreemode" ] || err "F3.2n mode differs for $csf32ntreepath"
  if [ "$csf32ntreeblob" != "-" ]; then
    [ "$csf32ntreeactblob" = "$csf32ntreeblob" ] || err "F3.2n blob differs for $csf32ntreepath"
  fi
done

$GIT diff --name-only "$csf32nbase" "$head" > "$tmpdir/cs.f32n.paths.raw"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n cumulative path enumeration failed"
LC_ALL=C sort "$tmpdir/cs.f32n.paths.raw" > "$tmpdir/cs.f32n.paths.act"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n cumulative path sort failed"
cat > "$tmpdir/cs.f32n.paths.exp" <<'QK_CS_F32N_PATHS_EOF' || err "F3.2n cumulative path fixture failed"
docs/DECISION-LOG.md
docs/f3/F3.2N-D11-Q002-SD-ATTACHMENT-EPOCH-OWNER-DIRECTION-RECORD.md
tools/verify-current-stage.sh
tools/verify-host-boundary.sh
QK_CS_F32N_PATHS_EOF
diff "$tmpdir/cs.f32n.paths.exp" "$tmpdir/cs.f32n.paths.act" > "$tmpdir/cs.f32n.paths.diff" 2>&1
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n cumulative path set differs: $(cat "$tmpdir/cs.f32n.paths.diff")"

$GIT rev-parse "$csf32nbase:docs/DECISION-LOG.md" > "$tmpdir/cs.f32n.dlog.base.blob" 2>/dev/null
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n base Decision Log blob lookup failed"
printf '%s\n' "$csf32ndlogbaseblob" > "$tmpdir/cs.f32n.dlog.base.exp" || err "F3.2n base Decision Log fixture failed"
cmp -s "$tmpdir/cs.f32n.dlog.base.exp" "$tmpdir/cs.f32n.dlog.base.blob"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n base Decision Log blob differs"
$GIT rev-parse "$csf32na:docs/DECISION-LOG.md" > "$tmpdir/cs.f32n.dlog.a.blob" 2>/dev/null
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n A Decision Log blob lookup failed"
printf '%s\n' "$csf32ndlogablob" > "$tmpdir/cs.f32n.dlog.a.exp" || err "F3.2n A Decision Log fixture failed"
cmp -s "$tmpdir/cs.f32n.dlog.a.exp" "$tmpdir/cs.f32n.dlog.a.blob"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n A Decision Log blob differs"
$GIT rev-parse "$head:docs/DECISION-LOG.md" > "$tmpdir/cs.f32n.dlog.head.blob" 2>/dev/null
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n HEAD Decision Log blob lookup failed"
cmp -s "$tmpdir/cs.f32n.dlog.a.exp" "$tmpdir/cs.f32n.dlog.head.blob"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n Decision Log changed after exact A"
$GIT show "$csf32nbase:docs/DECISION-LOG.md" > "$tmpdir/cs.f32n.dlog.base" 2>/dev/null
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "cannot read F3.2n base Decision Log"
$GIT show "$csf32na:docs/DECISION-LOG.md" > "$tmpdir/cs.f32n.dlog.a" 2>/dev/null
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "cannot read F3.2n A Decision Log"
wc -c < "$tmpdir/cs.f32n.dlog.base" > "$tmpdir/cs.f32n.dlog.bytes.raw"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n base Decision Log byte count failed"
tr -d '[:space:]' < "$tmpdir/cs.f32n.dlog.bytes.raw" > "$tmpdir/cs.f32n.dlog.bytes"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n Decision Log byte-count normalization failed"
csf32nbasebytes=$(cat "$tmpdir/cs.f32n.dlog.bytes")
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n Decision Log byte-count read failed"
case $csf32nbasebytes in ''|*[!0-9]*) err "F3.2n Decision Log base byte count is non-numeric" ;; esac
dd if="$tmpdir/cs.f32n.dlog.a" bs=1 count="$csf32nbasebytes" > "$tmpdir/cs.f32n.dlog.prefix" 2> "$tmpdir/cs.f32n.dlog.dd.err"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n Decision Log prefix read failed"
cmp -s "$tmpdir/cs.f32n.dlog.base" "$tmpdir/cs.f32n.dlog.prefix"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n Decision Log is not an exact append"

$GIT show "$csf32nb:$F32NREC" > "$tmpdir/cs.f32n.record.b" 2>/dev/null
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "cannot read F3.2n record from B"
cmp -s "$tmpdir/cs.f32n.record.b" "$F32NREC"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "active F3.2n record differs from exact B blob"
$GIT show "$F32M_C:$F32MPKT" > "$tmpdir/cs.f32n.packet.base" 2>/dev/null
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "cannot read immutable F3.2m packet from F3.2n base"
cmp -s "$tmpdir/cs.f32n.packet.base" "$F32MPKT"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "active F3.2m packet differs from F3.2n source base"

sed -n '/^### QK-AUTH-F3\.2N-D11-Q002-SD-ATTACHMENT-EPOCH-DIR-REC-001 /,$p' "$tmpdir/cs.f32n.dlog.a" > "$tmpdir/cs.f32n.dlog.section"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n Decision Log section extraction failed"
sed -n '/^- \*\*Owner words exactly (literal six-paragraph block follows):\*\*$/,/^- \*\*Published source boundary:\*\*/p' \
  "$tmpdir/cs.f32n.dlog.section" > "$tmpdir/cs.f32n.dlog.owner.framed"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n Decision Log owner block extraction failed"
sed -n '/^## Owner words exactly$/,/^## Exact direction map$/p' "$F32NREC" > "$tmpdir/cs.f32n.record.owner.framed"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n record owner block extraction failed"
[ "$(awk 'END {print NR+0}' "$tmpdir/cs.f32n.dlog.owner.framed")" = 15 ] \
  || err "F3.2n Decision Log owner block framing differs"
[ "$(awk 'END {print NR+0}' "$tmpdir/cs.f32n.record.owner.framed")" = 15 ] \
  || err "F3.2n record owner block framing differs"
sed -n '3,13p' "$tmpdir/cs.f32n.dlog.owner.framed" > "$tmpdir/cs.f32n.dlog.owner"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n Decision Log owner transcript deframing failed"
sed -n '3,13p' "$tmpdir/cs.f32n.record.owner.framed" > "$tmpdir/cs.f32n.record.owner"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n record owner transcript deframing failed"
cmp -s "$tmpdir/cs.f32n.dlog.owner" "$tmpdir/cs.f32n.record.owner"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n six-paragraph owner transcript differs between exact A and record"

sed -n 's/^| F32M-D11-Q002-OPT-001 | PROPOSED — UNSELECTED | RECOMMENDED — NOT SELECTED | \(.*\) |$/\1/p' \
  "$tmpdir/cs.f32n.packet.base" > "$tmpdir/cs.f32n.selected.packet"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n selected source-cell extraction failed"
[ "$(awk 'END {print NR+0}' "$tmpdir/cs.f32n.selected.packet")" = 1 ] \
  || err "F3.2n selected source-cell extraction is not exactly one line"
sed -n '/^## Selected option text exactly$/,/^## Conditional future direction$/p' "$F32NREC" \
  > "$tmpdir/cs.f32n.selected.record.framed"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n selected-text block extraction failed"
[ "$(awk 'END {print NR+0}' "$tmpdir/cs.f32n.selected.record.framed")" = 5 ] \
  || err "F3.2n selected-text block framing differs"
sed -n '3p' "$tmpdir/cs.f32n.selected.record.framed" > "$tmpdir/cs.f32n.selected.record"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n selected-text deframing failed"
cmp -s "$tmpdir/cs.f32n.selected.packet" "$tmpdir/cs.f32n.selected.record"
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n selected text differs from exact unquoted F3.2m OPT-001 disposition cell"
...
for csf32nline in \
  ...
  csf32nlinehits=$(grep -cFx -e "$csf32nline" "$F32NREC")
  csf32nrc=$?
  [ "$csf32nrc" -le 1 ] || err "F3.2n exact semantic-line scan failed"
  [ "$csf32nlinehits" = 1 ] || err "F3.2n exact semantic line differs or duplicates: $csf32nline"
done
forbid "F3.2n current-stage check rejects stale substitute-medium wording" \
  -F 'substitute-medium output' "$F32NREC"
forbid "F3.2n current-stage check rejects the overbroad stale retry-policy wording" \
  -F 'selects no retry, continuation, fallback, or re-signing policy.' "$F32NREC"
csf32npacketoptions=$(grep -c '^| F32M-D11-Q002-OPT-' "$tmpdir/cs.f32n.packet.base")
csf32nrc=$?
[ "$csf32nrc" -le 1 ] || err "F3.2n historical packet option-row count failed"
[ "$csf32npacketoptions" = 3 ] || err "F3.2n historical packet must retain exactly three option rows"
csf32npacketunselected=$(grep -c '^| F32M-D11-Q002-OPT-.*| PROPOSED — UNSELECTED |' "$tmpdir/cs.f32n.packet.base")
csf32nrc=$?
[ "$csf32nrc" -le 1 ] || err "F3.2n historical packet unselected-row count failed"
[ "$csf32npacketunselected" = 3 ] || err "F3.2n historical packet must retain three PROPOSED — UNSELECTED rows"

for csf32nsource in \
  'ARCHITECTURE.md 71e3f95adf541560ff5d067030f78dfd088d1ec1' \
  'docs/BUILD-ROADMAP.md b2008af9c2028ba819779c1f9860138ceb1671aa' \
  'docs/REQUIREMENTS.md 2cf8a34aa5621dcea114e2d5ec3dabc2b79aef84' \
  'docs/RESOURCE-BUDGETS.md 09815b0a75c26a118f80c5cec008e695d319900b' \
  'docs/TEST-ARCHITECTURE.md ecb3867ca2842f6d011288d3dcd94d2b1997e516' \
  'docs/MATURITY-GATES.md 5b6b67b7ec44f1460f31a02803e6f6db4f9386d1' \
  ...
  'docs/f3/F3.2D-D11-Q001-OWNER-CLARIFICATION-RECORD.md a996a6d7c2bfa3a15109085475868410fe354422' \
  'docs/f3/F3.2M-D11-Q002-SD-ATTACHMENT-EPOCH-DECISION-PACKET.md 11714544ea89b3475d65a8a5634b7fdab65f5972'; do
  csf32npath=${csf32nsource%% *}
  csf32nblob=${csf32nsource#* }
  printf '%s\n' "$csf32nblob" > "$tmpdir/cs.f32n.source.exp" || err "F3.2n protected-source fixture failed"
  $GIT rev-parse "$csf32nbase:$csf32npath" > "$tmpdir/cs.f32n.source.base" 2>/dev/null
  csf32nrc=$?
  [ "$csf32nrc" -eq 0 ] || err "F3.2n source-base lookup failed for $csf32npath"
  cmp -s "$tmpdir/cs.f32n.source.exp" "$tmpdir/cs.f32n.source.base"
  csf32nrc=$?
  [ "$csf32nrc" -eq 0 ] || err "F3.2n source-base blob differs for $csf32npath"
  $GIT rev-parse "$head:$csf32npath" > "$tmpdir/cs.f32n.source.head" 2>/dev/null
  csf32nrc=$?
  [ "$csf32nrc" -eq 0 ] || err "F3.2n active protected-source lookup failed for $csf32npath"
  cmp -s "$tmpdir/cs.f32n.source.exp" "$tmpdir/cs.f32n.source.head"
  csf32nrc=$?
  [ "$csf32nrc" -eq 0 ] || err "F3.2n protected source differs for $csf32npath"
done

$GIT show "$csf32nb:tools/verify-host-boundary.sh" > "$tmpdir/cs.f32n.host.b" 2>/dev/null
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "cannot read F3.2n host verifier from B"
cmp -s "$tmpdir/cs.f32n.host.b" tools/verify-host-boundary.sh
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "active host verifier differs from exact F3.2n B blob"
csf32nprovenanceblocks=$(awk '
  ...
  { state=0 }
  END { print found+0 }' "$tmpdir/cs.f32n.host.b")
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n current-stage provenance-block scan failed"
[ "$csf32nprovenanceblocks" = 1 ] || err "F3.2n current-stage truthful F3.2m provenance block differs, duplicates, or is non-contiguous"
forbid "F3.2n current-stage check rejects stale F3.2m present-tense provenance wording" \
  -E '^# The exact F3[.]2l C snapshot is published and immutable[.] F3[.]2m is a$' "$tmpdir/cs.f32n.host.b"
forbid "F3.2n current-stage check rejects stale F3.2m local/unpublished provenance wording" \
  -E '^# local[/]unpublished, non-binding, non-enacting decision input only[.]$' "$tmpdir/cs.f32n.host.b"
for csf32nhostline in \
  ...
  'F3.2n HEAD is neither exact intermediate B nor exact final C by bodyless message' \
  'F3.2n A changes a path other than docs/DECISION-LOG.md' \
  'F3.2n B path set differs from exact record/host pair' \
  'F3.2n C changes a path other than tools/verify-current-stage.sh' \
  'F3.2n cumulative path set differs from exact stage partition' \
  'F3.2n six-paragraph owner transcript differs between exact A and record' \
  'F3.2n selected text differs from exact unquoted F3.2m OPT-001 disposition cell' \
  'F3.2n record whole blob differs' \
  'F3.2n immutable F3.2m source packet differs' \
  '- No continuation or substitute-media output is permitted under that approval.' \
  '- Unchanged-epoch retry remains open under `F32C-D11-Q-005`; this record selects no unchanged-epoch retry policy. The published F3.2d Q-001 clarification remains byte-identical and unchanged.' \
  'F3.2n truthful F3.2m provenance block differs, duplicates, or is non-contiguous' \
  'F3.2n host verifier restores stale F3.2m present-tense provenance wording' \
  'F3.2n host verifier restores stale F3.2m local/unpublished provenance wording' \
  'F3.2n record restores stale substitute-medium wording' \
  'F3.2n record restores the overbroad stale retry-policy wording'; do
  grep -F -e "$csf32nhostline" "$tmpdir/cs.f32n.host.b" > /dev/null 2>&1
  csf32nrc=$?
  [ "$csf32nrc" -eq 0 ] || err "F3.2n host mechanism missing: $csf32nhostline"
done
forbid "F3.2n host stage block depends on origin or an upstream tracking ref" \
  -E 'refs/remotes/origin|origin/main|@\{u\}' "$tmpdir/cs.f32n.host.b"
sh tools/verify-host-boundary.sh > "$tmpdir/cs.f32n.host.out" 2>&1
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n strict host verifier failed: $(cat "$tmpdir/cs.f32n.host.out")"
grep -x 'HOST BOOTSTRAP SCOPE PASS' "$tmpdir/cs.f32n.host.out" > /dev/null 2>&1
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n host verifier did not emit its sole exact PASS line"
csf32nhostlines=$(awk 'END {print NR+0}' "$tmpdir/cs.f32n.host.out")
csf32nrc=$?
[ "$csf32nrc" -eq 0 ] || err "F3.2n host verifier output count failed"
[ "$csf32nhostlines" = 1 ] || err "F3.2n host verifier emitted extra output"

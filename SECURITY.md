...
This repository is **pre-production** and **unsafe for funds**. Its bounded HOST-only Rust evidence now covers structural PSBT-v0 parsing and M5 canonical serialization, M8 existing-signature verification, public BIP32/xpub and paired-descriptor foundations, descriptor ownership/change proof, recipient-script facts, D-09 review binding, and M15 final-only canonical PSBT emission by inserting externally produced signatures. The only secp256k1 production calls remain the five verification/public-derivation functions exposed by qk-secp; no Rust signing function or private-key input exists.

M15 requires the retained approved review hash and current cycle token, inserts one authenticated descriptor-role signature record at a time, reparses and revalidates after each insertion, and returns bytes only when every input is threshold-complete. It does not generate signatures, finalize or extract a transaction, broadcast, or perform media output. There is still no wallet product, seed/xprv handling, private derivation, arbitrary descriptor language, address renderer, target UI/runtime, or production evidence. Do not use anything here to store, receive, or move real bitcoin.

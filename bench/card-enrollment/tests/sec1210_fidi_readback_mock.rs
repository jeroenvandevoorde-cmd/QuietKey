...
fn run_with_slow_diagnostic(marker: &'static [u8]) -> (Sec1210FidiReadbackSummary, usize, String) {
    ...
    let mut transcript = Sec1210FidiReadbackTranscript::new(SlowDiagnostic {
        bytes: vec![],
        clock: mock.extra_clock.clone(),
        marker,
    });
    let utc = "2026-09-12T00:00:00Z";
    let metadata = Sec1210FidiReadbackMetadata::new(
        "a".repeat(40),
        utc.into(),
        "RIG-HOST-PI3B-01",
        "J3R180-03",
        std::env::temp_dir().join(sec1210_fidi_readback_output_basename(utc)),
    )
    .unwrap();
    let summary = run_sec1210_fidi_readback(&metadata, &mut mock, &mut transcript);
    (
        summary,
        mock.writes.len(),
...
fn accepted_wire_diagnostics_cannot_renew_the_command_deadline() {
    let (summary, writes, text) = run_with_slow_diagnostic(b"read.5.validation_ms=");
    assert_eq!(
        summary.failure.map(|e| e.name()),
        Some("Sec1210DeadlineExceeded")
    );
    ...
    assert!(summary.ifs_accepted && summary.local_handle_released);
    assert!(text.contains("read.5.comparison=PASS\n"));
...
fn complete_unchained_transcripts_agree_and_counts_are_observed() {
    for width in [1, 2, 8, 274] {
        ...
        for line in [
            ...
            "t1.6.send_sequence=0",
            "t1.6.receive_sequence=0",
        ] {
...
fn unsolicited_ifs_and_wtx_after_activation_remain_terminal() {
    for (payload, error) in [
        ...
        let mut m = Mock::new(274);
        m.reads[5].0 = ccid(0x80, 6, 0, 0, &payload);
        let (s, t) = run(&mut m);
        assert_eq!(s.failure.unwrap().name(), error, "{t}");
...
fn activated_transport_bound_does_not_expand_the_golden_response() {
    ...
    m.reads[5].0 = ccid(0x80, 6, 0, 0, &block(0, &[0; 254]));
    let (s, t) = run(&mut m);
    assert_eq!(s.failure.unwrap().name(), "T1ResponseLengthRejected", "{t}");
    assert!(t.contains("observation.5=Transfer sequence=6 payload_bytes=258\n"));
    assert!(t.contains("read.5.comparison=PASS\n"));
...
fn block_above_258_bytes_is_rejected_after_successful_activation() {
    let mut m = Mock::new(274);
    m.reads[5].0 = ccid(0x80, 6, 0, 0, &block(0, &[0; 255]));
    let (s, t) = run(&mut m);
    assert_eq!(s.failure.unwrap().name(), "T1BlockLengthRejected", "{t}");
    assert!(t.contains("observation.5=Transfer sequence=6 payload_bytes=259\n"));
    assert!(t.contains("read.5.comparison=PASS\n"));
    assert_eq!(s.apdu_response_count, 0);
    assert!(s.ifs_accepted && s.local_handle_released);
    assert_eq!(m.writes.len(), 6);
...
fn zero_length_reads_and_coalesced_ifs_event_remain_reproducible() {
    let mut m = Mock::new(274);
    let response = m.reads[4].0.clone();
    m.reads[4].0 = [vec![0x50, 3], response].concat();
    m.reads.insert(4, (vec![], 0));
    let (s, t) = run(&mut m);
    assert_eq!(s.failure, None, "{t}");
    assert_eq!(s.event_count, 1);
    assert!(t.contains("read.4.elapsed_ms=10\nread.4.monotonic_ms=54\nread.4.rx_hex=\n"));
    let indices: Vec<usize> = t
        .lines()
        .filter_map(|l| l.strip_prefix("observation."))
        .map(|l| l.split('=').next().unwrap().parse().unwrap())
        .collect();
    assert_eq!(indices, (0..39).collect::<Vec<_>>());
}
...
fn late_and_partial_ifs_responses_never_reach_an_apdu() {
    for partial in [false, true] {
        ...
        let response = m.reads[4].0.clone();
        if partial {
            m.reads[4] = (response[..4].to_vec(), 1);
            m.reads.insert(5, (response[4..].to_vec(), 5000));
        } else {
            m.reads[4].1 = 5000;
        }
        let (s, t) = run(&mut m);
        ...
        assert_eq!(m.writes.len(), 5);
        assert!(!s.ifs_accepted);
        assert_eq!(s.captured_rx_bytes, 99);
        assert!(s.local_handle_released);
    }
}
...
fn ifs_transcript_work_is_inside_its_absolute_budget() {
    ...
    let mut m = Mock::new(274);
    let mut transcript = Sec1210FidiReadbackTranscript::new(SlowWriter {
    ...
    .unwrap();
    let s = run_sec1210_fidi_readback(&metadata, &mut m, &mut transcript);
...
fn transcript_overflow_retains_first_failure_and_releases_without_retry() {
    ...
    let (s, t) = run(&mut m);
    assert_eq!(s.failure, Some(Sec1210FidiReadbackError::TranscriptLimit));
    assert!(s.local_handle_released);
    assert_eq!(m.writes.len(), 1);
    assert!(t.len() <= MAX_SEC1210_FIDI_READBACK_TRANSCRIPT_BYTES);
    assert!(t.contains("transcript_overflow=TRUE\n"));
    assert!(t.ends_with("first_failure=Sec1210FidiReadbackTranscriptLimit\nresult=Sec1210FidiReadbackTranscriptLimit\n"));
}
...
fn malformed_ifs_rejection_survives_a_diagnostic_write_failure() {
    ...
    let mut m = Mock::new(274);
    m.reads[4].0 = ccid(0x80, 5, 0, 0, &block(0xe1, &[0xfd]));
    let utc = "2026-09-12T00:00:00Z";
    let metadata = Sec1210FidiReadbackMetadata::new(
        "a".repeat(40),
        utc.into(),
    ...
    .unwrap();
    let s = run_sec1210_fidi_readback(
        &metadata,
        &mut m,
        &mut Sec1210FidiReadbackTranscript::new(Fail),
    );
...
fn stream_a(chunk_size: usize) -> (Vec<Vec<u8>>, Vec<Vec<u8>>) {
    let mut tx = vec![
        ...
        ccid(0x61, 4, 1, 0, &[0x18, 0x10, 0xff, 0x4d, 0, 0xfe, 0]),
        ccid(0x6f, 5, 0, 0, &block(0xc1, &[0xfe])),
    ];
    let mut rx = vec![
        ccid(0x81, 1, 1, 1, &[]),
        ccid(0x80, 2, 0, 0, &unhex("3bd518ff8191fe1fc38073c821100a")),
        ccid(0x82, 3, 0, 1, &[0x11, 0x10, 0, 0x4d, 0, 0xfe, 0]),
        ccid(0x82, 4, 0, 1, &[0x18, 0x10, 0xff, 0x4d, 0, 0xfe, 0]),
        ccid(0x80, 5, 0, 0, &block(0xe1, &[0xfe])),
    ];
    let mut ns = 0;
    let mut nr = 0;
...
fn stream_b(chunk_size: usize) -> (Vec<Vec<u8>>, Vec<Vec<u8>>) {
    ...
    let mut requests = vec![
        ...
        unhex("03066200000000000202000067"),
        unhex("03066c0000000000030000006a"),
        unhex("0306610700000000040100001810ff4d00fe0022"),
        unhex("03066f05000000000500000000c101fe3e6a"),
    ];
    let mut responses = vec![
        ...
        unhex("0306820700000000040000011810ff4d00fe00c1"),
        unhex("03068005000000000500000000e101fe1e85"),
    ];
    // Trailer bytes are independently recomputed after the literal construction.
...
impl Sec1210Transport for Mock {
    ...
    fn write_once(&mut self, b: &[u8]) -> Result<usize, Sec1210Error> {
        ...
        if self.failure == Some("partial")
            || (self.failure == Some("ifs-partial") && self.writes.len() == 5)
            || (self.failure == Some("set-parameters-partial") && self.writes.len() == 4)
        {
            Ok(b.len() - 1)
        } else {
            Ok(b.len())
...
fn run(m: &mut Mock) -> (Sec1210FidiReadbackSummary, String) {
    let utc = "2026-09-12T00:00:00Z";
    let metadata = Sec1210FidiReadbackMetadata::new(
        "a".repeat(40),
        utc.into(),
    ...
    .unwrap();
    let mut transcript = Sec1210FidiReadbackTranscript::new(Vec::new());
    let summary = run_sec1210_fidi_readback(&metadata, m, &mut transcript);
...
fn complete_chained_public_golden_stream_is_reconstructed_without_device() {
    ...
    assert!(t.contains("tool_version=0.0.12\n"));
    assert!(t.contains("parameters_hex=1110004d00fe00\n"));
    assert!(t.contains("QK-LIM-BENCH-T1-FIDI-READBACK-TRANSCRIPT-V1=262144\n"));
...
fn response_prefix_mismatch_stops_before_another_ack() {
    let mut m = Mock::new(274);
    // INFO is the third APDU, first chained response (command8).
    let mut bad = m.reads[7].0.clone();
    bad[15] ^= 1;
    let n = bad.len();
    bad[n - 2] ^= 1;
    ...
    assert_eq!(
        s.failure,
        Some(Sec1210FidiReadbackError::T1(qk_t1::Error::ResponseMismatch)),
        "{t}"
    );
    assert_eq!(m.writes.len(), 8);
...
fn late_raw_bytes_are_retained_and_never_acknowledged() {
    ...
    assert_eq!(
        s.failure,
        Some(Sec1210FidiReadbackError::Wire(
...
fn recording_raw_bytes_cannot_bypass_the_absolute_response_deadline() {
    ...
    let mut m = Mock::new(274);
    let mut t = Sec1210FidiReadbackTranscript::new(SlowWriter {
        bytes: vec![],
        clock: m.extra_clock.clone(),
    });
    let utc = "2026-09-12T00:00:00Z";
    ...
    .unwrap();
    let s = run_sec1210_fidi_readback(&metadata, &mut m, &mut t);
    assert_eq!(s.failure.unwrap().name(), "Sec1210DeadlineExceeded");
    assert_eq!(m.writes.len(), 1);
    assert_eq!(s.captured_rx_bytes, 13);
...
fn chained_timely_ccid_responses_cannot_renew_the_apdu_deadline() {
    ...
    assert_eq!(s.apdu_response_count, 2);
    assert_eq!(m.writes.len(), 15); // Seven earlier commands plus eight timely exchanges.
}
...
fn diagnostic_writer_failure_cannot_replace_an_observed_protocol_rejection() {
    ...
    for (wire_error, marker, expected) in [
        ...
        (false, &b"t1.6.comparison="[..], "T1SequenceRejected"),
    ] {
        let mut m = Mock::new(274);
        if wire_error {
            m.reads[0].0 = ccid(0x81, 1, 0x81, 0, &[]);
        } else {
            m.reads[5].0 = ccid(0x80, 6, 0, 0, &block(0x40, &[0x90, 0]));
        }
        let mut t = Sec1210FidiReadbackTranscript::new(FailWriter(marker));
        let utc = "2026-09-12T00:00:00Z";
        let metadata = Sec1210FidiReadbackMetadata::new(
            "a".repeat(40),
            utc.into(),
        ...
        assert!(s.local_handle_released);
        assert_eq!(m.writes.len(), if wire_error { 1 } else { 6 });
    }
}

fn patched(mut bytes: Vec<u8>, index: usize, value: u8) -> Vec<u8> {
    bytes[index] = value;
    let last = bytes.len() - 1;
    bytes[last] = bytes[..last].iter().fold(0, |a, b| a ^ b);
    bytes
}

#[test]
fn exact_parameters_are_separate_from_baseline_and_not_full_frames() {
    let (s, t) = run(&mut Mock::new(8));
    assert_eq!(s.failure, None, "{t}");
    assert!(s.set_parameters_accepted && s.ifs_accepted);
    for line in [
        "command.4.request_hex=0306610700000000040100001810ff4d00fe0022",
        "command.5.request_hex=03066f05000000000500000000c101fe3e6a",
        "parameters_hex=1110004d00fe00",
        "set_parameters.request_hex=1810ff4d00fe00",
        "set_parameters.response_hex=1810ff4d00fe00",
        "set_parameters.bStatus=00",
        "set_parameters.bError=00",
        "set_parameters.bProtocolNum=01",
        "set_parameters.comparison=PASS",
        "set_parameters_accepted=PASS",
    ] {
        assert_eq!(
            t.lines().filter(|actual| *actual == line).count(),
            1,
            "{line}"
        );
    }
    assert_eq!(
        t.lines()
            .filter(|line| line.starts_with("parameters_hex="))
            .count(),
        1
    );
    assert!(
        t.find("set_parameters.comparison=PASS").unwrap() < t.find("ifs.request_hex=").unwrap()
    );
}

#[test]
fn every_changed_parameter_byte_is_an_exact_echo_rejection_without_ifs() {
    let original = [0x18, 0x10, 0xff, 0x4d, 0, 0xfe, 0];
    for index in 0..original.len() {
        let mut altered = original;
        altered[index] ^= 1;
        let mut mock = Mock::new(274);
        mock.reads[3].0 = ccid(0x82, 4, 0, 1, &altered);
        let (s, t) = run(&mut mock);
        assert_eq!(
        ...
        assert_eq!(s.response_count, 3);
        assert_eq!(s.apdu_transmit_count, 0);
        assert!(!s.set_parameters_accepted && !s.ifs_accepted);
        assert!(s.local_handle_released);
        assert!(!t.contains("ifs.request_hex="));
        assert!(t.contains("set_parameters.comparison=Sec1210SetParametersEchoRejected\n"));
        assert!(t.contains("set_parameters_accepted=FALSE\n"));
    }
}

#[test]
fn status_shape_and_protocol_precedence_retains_actual_rejected_reply() {
    let exact = ccid(0x82, 4, 0, 1, &[0x18, 0x10, 0xff, 0x4d, 0, 0xfe, 0]);
    let mut bad_checksum = exact.clone();
    *bad_checksum.last_mut().unwrap() ^= 1;
    let cases = [
        (patched(exact.clone(), 7, 1), "Sec1210SlotRejected", true),
        (
            patched(exact.clone(), 8, 5),
            "Sec1210SequenceRejected",
            true,
        ),
        (patched(exact.clone(), 9, 4), "Sec1210StatusReserved", true),
        (
            patched(exact.clone(), 2, 0x80),
            "Sec1210ResponseTypeRejected",
            true,
        ),
        (
            patched(exact.clone(), 10, 10),
            "Sec1210StatusErrorRejected",
            true,
        ),
        (
            patched(exact.clone(), 9, 1),
            "Sec1210IccStatusRejected",
            true,
        ),
        (patched(exact.clone(), 9, 2), "Sec1210CardAbsent", true),
        (
            patched(exact.clone(), 11, 0),
            "Sec1210ProtocolRejected",
            true,
        ),
        (ccid(0x82, 4, 0, 1, &[]), "Sec1210PayloadRejected", true),
        (ccid(0x82, 4, 0, 0, &[1; 8]), "Sec1210PayloadRejected", true),
        (
            patched(ccid(0x82, 4, 0x40, 1, &[]), 10, 10),
            "Sec1210CommandFailed",
            true,
        ),
        (
            patched(ccid(0x82, 4, 0x40, 0, &[0; 7]), 10, 15),
            "Sec1210CommandFailed",
            true,
        ),
        (
            patched(ccid(0x81, 4, 0x80, 0, &[]), 10, 7),
            "Sec1210TimeExtensionRejected",
            true,
        ),
        (bad_checksum, "Sec1210ChecksumRejected", false),
        (
            [exact.clone(), vec![0x50, 3]].concat(),
            "Sec1210TrailingData",
            true,
        ),
    ];
    for (reply, error, decoded) in cases {
        let mut mock = Mock::new(274);
        mock.reads[3].0 = reply.clone();
        let (s, t) = run(&mut mock);
        assert_eq!(s.failure.unwrap().name(), error, "{t}");
        assert_eq!(mock.writes.len(), 4);
        assert!(!s.set_parameters_accepted && !s.ifs_accepted);
        assert_eq!(s.apdu_transmit_count, 0);
        assert!(s.local_handle_released);
        assert!(!t.contains("command.5.request_hex="));
        assert!(t.contains(&format!("set_parameters.comparison={error}\n")));
        assert_eq!(t.contains("set_parameters.response_hex="), decoded, "{t}");
        if decoded {
            assert!(t.contains(&format!("set_parameters.bError={:02x}\n", reply[10])));
            assert!(t.contains(&format!("set_parameters.bStatus={:02x}\n", reply[9])));
            assert!(t.contains(&format!("set_parameters.bProtocolNum={:02x}\n", reply[11])));
        }
        if reply[3] == 0 && decoded {
            assert!(t.contains("set_parameters.response_hex=\n"), "{t}");
        }
        if reply[3] == 8 && decoded {
            assert!(
                t.contains("set_parameters.response_hex=0101010101010101\n"),
                "{t}"
            );
        }
    }
...
#[test]
fn command_failure_survives_every_later_set_parameters_diagnostic_failure() {
    struct Fail(&'static [u8]);
    impl std::io::Write for Fail {
        fn write(&mut self, bytes: &[u8]) -> std::io::Result<usize> {
            if bytes.starts_with(self.0) {
                return Err(std::io::Error::other("injected later diagnostic failure"));
            }
            Ok(bytes.len())
        }
        fn flush(&mut self) -> std::io::Result<()> {
    ...
    for marker in [
        &b"read.3.validation_ms="[..],
        &b"set_parameters.bStatus="[..],
        &b"set_parameters.bError="[..],
        &b"set_parameters.bProtocolNum="[..],
        &b"set_parameters.response_hex="[..],
        &b"set_parameters.comparison="[..],
    ] {
        let mut mock = Mock::new(274);
        mock.reads[3].0 = patched(ccid(0x82, 4, 0x40, 1, &[]), 10, 10);
        ...
        let s = run_sec1210_fidi_readback(
            &metadata,
            &mut mock,
            &mut Sec1210FidiReadbackTranscript::new(Fail(marker)),
        );
        assert_eq!(s.failure.unwrap().name(), "Sec1210CommandFailed");
        assert!(!s.set_parameters_accepted && !s.ifs_accepted);
        assert_eq!(mock.writes.len(), 4);
        assert!(s.local_handle_released);
    }
...
#[test]
fn set_parameters_alone_is_not_pass_and_its_diagnostics_keep_the_deadline() {
    let (s, writes, t) = run_with_slow_diagnostic(b"set_parameters.comparison=");
    assert_eq!(s.failure.unwrap().name(), "Sec1210DeadlineExceeded");
    assert_eq!(writes, 4);
    assert!(s.set_parameters_accepted && !s.ifs_accepted);
    assert_eq!(s.apdu_transmit_count, 0);
    assert!(t.contains("set_parameters.comparison=PASS\n"));
    assert!(!t.ends_with("result=PASS\n"));
    assert!(!t.contains("command.5.request_hex="));
}

#[test]
fn partial_late_and_zero_read_set_parameters_never_skip_the_echo_gate() {
    for partial in [false, true] {
        let mut mock = Mock::new(274);
        let reply = mock.reads[3].0.clone();
        if partial {
            mock.reads[3] = (reply[..4].to_vec(), 1);
            mock.reads.insert(4, (reply[4..].to_vec(), 5000));
        } else {
            mock.reads[3].1 = 5000;
        }
        let (s, t) = run(&mut mock);
        ...
        assert_eq!(mock.writes.len(), 4);
        assert!(!s.set_parameters_accepted && !s.ifs_accepted);
        assert_eq!(s.captured_rx_bytes, 81);
        assert!(!t.contains("set_parameters.response_hex="));
    }
    let mut mock = Mock::new(274);
    mock.failure = Some("set-parameters-partial");
    let (s, t) = run(&mut mock);
    assert_eq!(s.failure.unwrap().name(), "Sec1210PartialWrite", "{t}");
    assert_eq!(mock.writes.len(), 4);
    assert!(!s.set_parameters_accepted && !s.ifs_accepted);
    let mut mock = Mock::new(274);
    mock.reads[3].0 = [vec![0x50, 3], mock.reads[3].0.clone()].concat();
    mock.reads.insert(3, (vec![], 0));
    let (s, t) = run(&mut mock);
    assert_eq!(s.failure, None, "{t}");
    let indices: Vec<usize> = t
        .lines()

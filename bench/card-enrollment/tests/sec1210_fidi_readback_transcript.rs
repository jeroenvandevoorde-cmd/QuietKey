...
fn failed() -> Sec1210FidiReadbackSummary {
...
fn new_cap_is_independent_and_reserves_terminal_records() {
    ...
    assert!(s.ends_with("result=Sec1210FidiReadbackTranscriptLimit\n"));
}
#[test]
...
fn io_failure_is_sticky_and_preserves_the_written_prefix() {
    ...
    let mut t = Sec1210FidiReadbackTranscript::new(Limited { bytes: vec![] });
    assert_eq!(
        t.field("x", "1234"),
        Err(Sec1210FidiReadbackError::TranscriptIo)
    );
    assert_eq!(
        t.finish(&failed()),
        Err(Sec1210FidiReadbackError::TranscriptIo)
...
fn limit_values_match_the_qualified_leaves_without_reusing_probe_ids() {
    assert_eq!(
        FIDI_READBACK_LIMITS.map(|(_, n)| n),
        [
            ...
            MAX_SEC1210_FIDI_READBACK_TRANSCRIPT_BYTES as u64
        ]
    );
    let mut ids = FIDI_READBACK_LIMITS.map(|(id, _)| id);
    ids.sort();
    assert!(ids.windows(2).all(|w| w[0] != w[1]));
    assert_eq!(MAX_SITTING_TRANSCRIPT_BYTES, 32768);
    assert_eq!(qk_sec1210_wire::MAX_RECEIVED_BYTES, 4096);
    for (id, _) in FIDI_READBACK_LIMITS {

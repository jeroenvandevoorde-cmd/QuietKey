use qk_card_enrollment::{
    ...
    IDENTITY_TRANSCRIPT_VERSION, REGISTERED_J3R180_ATR, SELECT_DEFAULT_APPLICATION_COMMAND,
};
    ...
    IdentityRecord {
        ...
        exchanges: [
            IdentityExchange {
                ...
                response: Some(vec![0x6f, 0x03, 0x84, 0x01, 0x00, 0x90, 0x00]),
            },
            IdentityExchange {
...
fn canonical_identity_transcript_is_byte_exact() {
    ...
    let expected = concat!(
        ...
        "event.5=TransmitSelect:PASS\n",
        "event.6=ReceiveSelect:PASS\n",
        "event.7=TransmitCardRecognition:PASS\n",
        "event.8=ReceiveCardRecognition:PASS\n",
        "event.9=TransmitCplc:PASS\n",
        "event.10=ReceiveCplc:PASS\n",
        "event.11=Disconnect:PASS\n",
        "protocol=T1\n",
        "atr_hex=3bd518ff8191fe1fc38073c821100a\n",
        "apdu_tx_count=3\n",
        "apdu_rx_count=3\n",
        "apdu.0.tx_hex=00a4040000\n",
        "apdu.0.rx_hex=6f038401009000\n",
        "apdu.1.tx_hex=80ca006600\n",
        "apdu.1.rx_hex=66037301009000\n",
        "apdu.2.tx_hex=80ca9f7f00\n",
        "apdu.2.rx_hex=9f7f2a000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f202122232425262728299000\n",
        "disconnect=PASS\n",
        "result=PASS\n",
    )
    .as_bytes();
    assert_eq!(IDENTITY_TRANSCRIPT_VERSION, "QK-CARD-IDENTITY-V2");
    assert_eq!(IDENTITY_ALLOWLIST_ID, "QK-F8-IDENT-V2");
...
fn failed_transmit_records_request_without_inventing_a_response() {
    ...
    record.events[5].outcome =
        IdentityOutcome::Reject(qk_card_enrollment::IdentityError::SelectTransmitFailed);
    ...
    record.exchanges[2] = IdentityExchange::default();
    record.outcome =
        IdentityOutcome::Reject(qk_card_enrollment::IdentityError::SelectTransmitFailed);
    let text =
        String::from_utf8(encode_identity_transcript(&record).expect("transcript")).expect("ASCII");
    assert!(text.contains("apdu_tx_count=1\napdu_rx_count=0\n"));
    assert!(text.contains("apdu.0.tx_hex=00a4040000\napdu.0.rx_hex=NONE\n"));
    assert!(text.contains("apdu.1.tx_hex=NONE\napdu.1.rx_hex=NONE\n"));
    assert!(text.contains("apdu.2.tx_hex=NONE\napdu.2.rx_hex=NONE\n"));
    assert!(text.ends_with("result=SelectTransmitFailed\n"));
}
...
fn contradictory_failure_records_never_encode() {
    ...
    invented_response.exchanges[2] = IdentityExchange::default();
    ...
    oversized_response.events[6].outcome =
        IdentityOutcome::Reject(IdentityError::SelectResponseTooLong);
    ...
    oversized_response.exchanges[2] = IdentityExchange::default();
    oversized_response.outcome = IdentityOutcome::Reject(IdentityError::SelectResponseTooLong);

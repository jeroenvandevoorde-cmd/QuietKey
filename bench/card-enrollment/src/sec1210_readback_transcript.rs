//! QK-CARD-SITTING-V1 extension for readback only; old encoders are unchanged.
use crate::{
    Sec1210ReadbackError as E, Sec1210ReadbackMetadata, Sec1210ReadbackSummary,
    CANONICAL_CAP_BYTES, CANONICAL_CAP_SHA256, READBACK_LIMITS, READBACK_PLAN_SHA256,
    SEC1210_READBACK_TOOL_VERSION, SEC1210_STTY_ARGS, SEC1210_TTY, SITTING_APPLET_SOURCE_COMMIT,
};
use qk_sec1210_wire::ReadbackObservation;
use std::io::Write;

pub const MAX_SEC1210_READBACK_TRANSCRIPT_BYTES: usize = 262_144;
pub struct Sec1210ReadbackTranscript<W: Write> {
    writer: W,
    bytes: usize,
    broken: bool,
    capped: bool,
}
impl<W: Write> Sec1210ReadbackTranscript<W> {
    ...
    fn line(&mut self, line: &str, terminal: bool) -> Result<(), E> {
        ...
        let limit = MAX_SEC1210_READBACK_TRANSCRIPT_BYTES - if terminal { 0 } else { 2048 };
    ...
    pub fn header(&mut self, m: &Sec1210ReadbackMetadata) -> Result<(), E> {
        self.line("QK-CARD-SITTING-V1", false)?;
        for (name, value) in [
            ("visibility", "PRIVATE_CUSTODY_ONLY"),
            ("allowlist", "QK-DEC-167-SUP-003"),
            ("tool_version", SEC1210_READBACK_TOOL_VERSION),
            ("source_commit", m.source()),
            ("timestamp_utc", m.utc()),
            ("host_alias", "RIG-HOST-PI3B-01"),
            ("specimen_alias", "J3R180-03"),
            ("mode", "sec1210-readback"),
        ...
        for (id, value) in READBACK_LIMITS {
    ...
    pub fn finish(&mut self, s: &Sec1210ReadbackSummary) -> Result<(), E> {
        for (name, value) in [

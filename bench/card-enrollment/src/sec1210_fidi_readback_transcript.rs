//! QK-CARD-SITTING-V1 extension for the fixed Fi/Di and IFSD-254 readback lane.
use crate::{
    Sec1210FidiReadbackError as E, Sec1210FidiReadbackMetadata, Sec1210FidiReadbackSummary,
    CANONICAL_CAP_BYTES, CANONICAL_CAP_SHA256, FIDI_READBACK_LIMITS, READBACK_PLAN_SHA256,
    SEC1210_FIDI_READBACK_TOOL_VERSION, SEC1210_STTY_ARGS, SEC1210_TTY,
    SITTING_APPLET_SOURCE_COMMIT,
};
use qk_sec1210_wire::ReadbackObservation;
use std::io::Write;

pub const MAX_SEC1210_FIDI_READBACK_TRANSCRIPT_BYTES: usize = 262_144;
pub struct Sec1210FidiReadbackTranscript<W: Write> {
    writer: W,
    bytes: usize,
    broken: bool,
    capped: bool,
}
impl<W: Write> Sec1210FidiReadbackTranscript<W> {
    ...
    fn line(&mut self, line: &str, terminal: bool) -> Result<(), E> {
        ...
        let limit = MAX_SEC1210_FIDI_READBACK_TRANSCRIPT_BYTES - if terminal { 0 } else { 2048 };
    ...
    pub fn header(&mut self, m: &Sec1210FidiReadbackMetadata) -> Result<(), E> {
        self.line("QK-CARD-SITTING-V1", false)?;
        for (name, value) in [
            ("visibility", "PRIVATE_CUSTODY_ONLY"),
            ("allowlist", "QK-DEC-167-SUP-010"),
            ("tool_version", SEC1210_FIDI_READBACK_TOOL_VERSION),
            ("source_commit", m.source()),
            ("timestamp_utc", m.utc()),
            ("host_alias", "RIG-HOST-PI3B-01"),
            ("specimen_alias", "J3R180-03"),
            ("mode", "sec1210-fidi-readback"),
        ...
        for (id, value) in FIDI_READBACK_LIMITS {
    ...
    pub fn set_parameters_reply(&mut self, reply: &qk_sec1210_wire::Response) -> Result<(), E> {
        ...
        // Retain actual candidate bytes: no padding or truncation on rejection.
        self.hex("set_parameters.response_hex", reply.payload())
    }
    pub fn observation(
        &mut self,
        index: usize,
        o: &ReadbackObservation,
        set_parameters: bool,
    ) -> Result<(), E> {
        let text = match o {
    ...
    pub fn finish(&mut self, s: &Sec1210FidiReadbackSummary) -> Result<(), E> {
        for (name, value) in [
        ...
        self.line(
            &format!(
                "set_parameters_accepted={}",
                if s.set_parameters_accepted {
                    "PASS"
                } else {
                    "FALSE"
                }
